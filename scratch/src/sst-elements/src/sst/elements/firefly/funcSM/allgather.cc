...
const char *AllgatherFuncSM::m_enumName[] = {FOREACH_ENUM(GENERATE_STRING)};

AllgatherFuncSM::AllgatherFuncSM(SST::Params &params)
    : FunctionSMInterface(params), m_event(NULL), m_seq(0) {
...
bool AllgatherFuncSM::setup(Retval &retval) {
  ...
  case SetupState::PostStartMsgRecv:
    ...
    src = m_info->getGroup(m_event->group)
              ->getMapping(calcSrc(m_setupState.offset));
  ...
  case SetupState::PostStageRecv:
    ...
    m_dbg.debug(CALL_INFO, 1, 0, "setup stage %d, src %d, dest %d\n",
                ...
                m_info->getGroup(m_event->group)
                    ->getMapping(m_dest[m_setupState.stage]));
    ...
    m_dbg.debug(CALL_INFO, 1, 0,
                " 2^stage %d, sendStart %d, recvStart %d "
                "numChunks %d\n",
                m_setupState.offset, m_sendStartChunk[m_setupState.stage],
                recvStartChunk, m_numChunks[m_setupState.stage]);

    initIoVec(ioVec, recvStartChunk, m_numChunks[m_setupState.stage],
              m_event->recvbuf.getBacking() != NULL);
  ...
  case SetupState::SendStartMsg:
    ...
    if (m_event->sendbuf.getBacking()) {
      memcpy(chunkPtr(m_rank), m_event->sendbuf.getBacking(),
             chunkSize(m_rank));
    }
...
void AllgatherFuncSM::handleEnterEvent(Retval &retval) {
  ...
  case SendData:
    initIoVec(ioVec, m_sendStartChunk[m_currentStage],
              ...
              m_event->sendbuf.getBacking() != NULL);

    {
      ...
      if (length <= m_smallCollectiveSize) {
        // int dest = m_info->getGroup(m_event->group)
        //                ->getMapping(m_dest[m_currentStage]);
        // int numVNs = 10;
        // vn = dest % numVNs;
        vn = m_smallCollectiveVN;
      }
      m_dbg.debug(
          CALL_INFO, 1, 0, "send stage %d, dest %d vn=%d\n", m_currentStage,
...
void AllgatherFuncSM::initIoVec(std::vector<IoVec> &ioVec, int startChunk,
                                int numChunks, bool backed) {
  ...
  while (countDown) {
    ...
    while (countDown && (unsigned char *)jjj.addr.getBacking() + jjj.len ==
                            chunkPtr(nextChunk)) {
    ...
    if (0 == countDown || (unsigned char *)jjj.addr.getBacking() + jjj.len !=
                              chunkPtr(nextChunk)) {

      ioVec.push_back(jjj);
      m_dbg.debug(CALL_INFO, 2, 0, "ioVec[%lu] ptr=%p len=%lu\n",
                  ioVec.size() - 1, ioVec[ioVec.size() - 1].addr.getBacking(),
