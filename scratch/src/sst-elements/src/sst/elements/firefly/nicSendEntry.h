...
public:
  SendEntryBase(int local_vNic, int streamNum)
      : m_local_vNic(local_vNic), m_streamNum(streamNum), m_isCtrl(false),
        m_isAck(false) {}
  ...
  virtual void copyOut(Output &dbg, int numBytes, FireflyNetworkEvent &event,
                       std::vector<MemOp> &vec) = 0;
...
public:
  CmdSendEntry(int local_vNic, int streamNum, NicCmdEvent *cmd,
      ...
      : SendEntryBase(local_vNic, streamNum), m_cmd(cmd), m_callback(callback) {
  ...
  void copyOut(Output &dbg, int numBytes, FireflyNetworkEvent &event,
  ...
  int route_id() override { return m_cmd->route_id; }
...
public:
  MsgSendEntry(int local_vNic, int streamNum, int dst_node, int dst_vNic)
      : SendEntryBase(local_vNic, streamNum), m_dst_node(dst_node),
        m_dst_vNic(dst_vNic) {}
...
public:
  GetOrgnEntry(int local_vNic, int streamNum, int dst_node, int dst_vNic,
               int rgnNum, int respKey, int vn)
