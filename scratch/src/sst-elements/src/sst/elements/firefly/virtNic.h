...
public:
  ...
  SST_ELI_REGISTER_SUBCOMPONENT(VirtNic, "firefly", "VirtNic",
                                SST_ELI_ELEMENT_VERSION(1, 0, 0), "",
                                SST::Firefly::VirtNic)

  SST_ELI_DOCUMENT_PORTS({"nic", "Connection to upper level nic", {}})
...
private:
  ...
  template <typename argT> class HandlerBase {
  public:
  ...
  template <typename T1, typename T2> class HandlerBase2Args {
  public:
...
public:
  ...
  public:
    ...
    bool operator()(argT data) { return (object->*member)(data); }
  };
  ...
  public:
    ...
    bool operator()(T1 arg1, T2 arg2) { return (object->*member)(arg1, arg2); }
  };
  ...
  int getCoreId() { return m_coreId; }

  int getRealNodeId() { return m_realNicId; }

  int getNodeId() { return m_realNicId * m_numCores + m_coreId; }

  bool isLocal(int nodeId, int nicsPerNode) {
    return ((nodeId / (m_numCores * nicsPerNode)) ==
            (m_realNicId / nicsPerNode));
  }
  ...
  void shmemWait(Hermes::Vaddr dest, Hermes::Shmem::WaitOp, Hermes::Value &,
                 Callback);
  ...
  void shmemPut(int node, Hermes::Vaddr dest, Hermes::Vaddr src, size_t len,
                Callback);
  void shmemGet(int node, Hermes::Vaddr dest, Hermes::Vaddr src, size_t len,
                Callback);
  ...
  void shmemCswap(int node, Hermes::Vaddr dest, Hermes::Value &cond,
                  Hermes::Value &value, CallbackV);
...
private:
  ...
  SimTime_t calcDelay() {
    ...
    m_dbg.debug(CALL_INFO, 2, 0,
                "curTime_ns=%" PRIu64 " delay_ns=%" PRIu64 "\n", curTime, ret);
  ...
  int calcNodeId(int nic, int vNic) { return nic * m_numCores + vNic; }
