...
public:
  Ctx(Output &output, RecvMachine &rm, int pid, int qsize)
    ...
    m_prefix = "@t:" + std::to_string(rm.nic().getNodeId()) +
               ":Nic::RecvMachine::Ctx" + std::to_string(pid) + "::@p():@l ";
  }
  ...
  EntryBase *findRecv(int srcNode, int srcPid, MsgHdr &hdr,
  ...
  Nic::Shmem *getShmem() { return m_rm.m_nic.m_shmem; }
  ...
  void needRecv(StreamBase *stream) {
    ...
    m_dbg.verbosePrefix(
        prefix(), CALL_INFO, 1, NIC_DBG_RECV_CTX,
        "pid %d blocked, srcNode=%d srcPid=%d m_blockedStreamQ.size()=%zu\n",
        stream->getSrcPid(), stream->getSrcNode(), stream->getSrcPid(),
        m_blockedStreamQ.size());

    m_rm.m_nic.notifyNeedRecv(stream->getMyPid(), stream->getSrcNode(),
                              stream->getSrcPid(), stream->length());
  }
  ...
  void calcNicMemDelay(int unit, std::vector<MemOp> *ops,
  ...
  void runSend(int num, SendEntryBase *entry) { m_rm.nic().qSendEntry(entry); }

  // this function is called by a Stream object, we don't want to delete
  // ourself, break the cycle by scheduling a callback
  void deleteStream(StreamBase *stream) {
    m_rm.nic().schedCallback([=]() {
      m_dbg.verbosePrefix(prefix(), CALL_INFO_LAMBDA, "deleteStream", 1,
                          NIC_DBG_RECV_CTX, "%p\n", stream);
  ...
  SimTime_t getRxMatchDelay() { return m_rm.m_rxMatchDelay; }

  int allocAckUnit() { return m_rm.m_nic.allocNicAckUnit(); }
  int allocRecvUnit() { return m_rm.m_nic.allocNicRecvUnit(m_pid); }
