...
#include "nicShmemRecvMachine.h"
  ...
  static StreamKey getStreamKey(FireflyNetworkEvent *ev) {
    ...
    value |= ev->getSrcPid() << NUM_PID_BITS;
    value |= (uint64_t)ev->getSrcNode() << (NUM_PID_BITS * 2);
    value += (uint64_t)ev->getSrcStream()
             << ((NUM_PID_BITS * 2) + NUM_NODE_BITS);
    return value;
  }
...
public:
  RecvMachine(Nic &nic, int vn, int numVnics, int nodeId, int verboseLevel,
              int verboseMask, int rxMatchDelay, int hostReadDelay,
              int maxQsize, int maxActiveStreams, int maxPendingPkts)
      : m_nic(nic), m_vn(vn), m_rxMatchDelay(rxMatchDelay),
        m_hostReadDelay(hostReadDelay), m_numActiveStreams(0),
        m_maxActiveStreams(maxActiveStreams), m_numMsgRcvd(0), m_clockLat(1),
        m_clocking(false), m_numPendingPkts(0),
        m_maxPendingPkts(maxPendingPkts) {
    char buffer[100];
    snprintf(buffer, 100, "@t:%d:Nic::RecvMachine::@p():@l vn=%d ", nodeId,
             m_vn);
...
private:
  ...
  void setNotify() {
    ...
    m_nic.m_linkRecvWidget->setNotify(
        std::bind(&Nic::RecvMachine::networkHasPkt, this), m_vn);
  }
  ...
  void clockHandler() {
    ...
    if (m_numPendingPkts < m_maxPendingPkts) {
      ...
      if (ev) {
        ++m_numPendingPkts;
        m_dbg.debug(CALL_INFO, 1, NIC_DBG_RECV_MACHINE,
                    "got packet numPendingPkts=%d\n", m_numPendingPkts);
        m_pktBuf[getPPI(ev)].push(ev);
      }
    } else {
      m_dbg.debug(CALL_INFO, 2, NIC_DBG_RECV_MACHINE,
                  "reached max buffered packets, numPendingPkts=%d\n",
                  m_numPendingPkts);
    }
    ...
    while (iter != m_pktBuf.end()) {
      ...
      m_dbg.debug(CALL_INFO, 2, NIC_DBG_RECV_MACHINE,
                  "packet from node=%d pid=%d for pid=%d %s %s PPI=0x%" PRIx64
                  " stream=%d\n",
                  ev->getSrcNode(), ev->getSrcPid(), ev->getDestPid(),
                  ev->isHdr() ? "hdr" : "", ev->isTail() ? "tail" : "",
                  getPPI(ev), ev->getSrcStream());

      if (ev->isCtrl()) {
        ++m_numActiveStreams;
        m_dbg.debug(CALL_INFO, 1, NIC_DBG_RECV_MACHINE,
                    "ctrl packet numActiveStreams=%d m_numPendingPkts=%d\n",
        ...
        if (m_numActiveStreams < m_maxActiveStreams) {
          ++m_numActiveStreams;
          m_dbg.debug(CALL_INFO, 1, NIC_DBG_RECV_MACHINE,
                      "new stream numActiveStreams=%d m_numPendingPkts=%d\n",
                      m_numActiveStreams, m_numPendingPkts - 1);
          processPkt(ev);
          --m_numPendingPkts;
          iter->second.pop();
          ...
          m_dbg.debug(CALL_INFO, 2, NIC_DBG_RECV_MACHINE,
                      "can't start new stream numActiveStreams=%d\n",
                      m_numActiveStreams);
        }
      } else {
        if (!m_streamMap[getStreamKey(ev)]->isBlocked()) {
          ...
          --m_numPendingPkts;
          m_dbg.debug(CALL_INFO, 1, NIC_DBG_RECV_MACHINE,
                      "stream consumed packet, m_numPendingPkts=%d\n",
                      m_numPendingPkts);
          iter->second.pop();
        } else {
      ...
      if (iter->second.empty()) {
        m_dbg.debug(CALL_INFO, 1, NIC_DBG_RECV_MACHINE,
                    "queue is empty clear pktBuf slot\n");
    ...
    if (m_pktBuf.empty() && !m_nic.m_linkControl->requestToReceive(m_vn)) {
      ...
      m_dbg.debug(
          CALL_INFO, 2, NIC_DBG_RECV_MACHINE,
          "schedule clockHandler m_numPendingPkts=%d m_numActiveStreams=%d\n",
          m_numPendingPkts, m_numActiveStreams);
      nic().schedCallback(std::bind(&Nic::RecvMachine::clockHandler, this),
                          m_clockLat);
    }
  }
  ...
  FireflyNetworkEvent *getNetworkEvent(int vn) {
    ...
    if (req) {
      ...
      FireflyNetworkEvent *event = static_cast<FireflyNetworkEvent *>(payload);
