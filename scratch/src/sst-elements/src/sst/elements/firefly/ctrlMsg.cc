...
API::API(ComponentId_t id, Params &params)
  ...
  m_mem = loadAnonymousSubComponent<Memory>("firefly.ctrlMsgMemory", "", 0,
...
API::~API() { delete m_mem; }

void API::setup() { m_processQueuesState->setup(); }
void API::finish() { m_processQueuesState->finish(); }

void API::setVars(Info *info, VirtNic *nic, Thornhill::MemoryHeapLink *mem,
                  Link *retLink) {
  m_info = info;
  m_memHeapLink = mem;
  ...
  nic->setNotifyNeedRecv(
      new VirtNic::Handler2Args<API, int, size_t>(this, &API::notifyNeedRecv));
...
void API::makeProgress() { m_processQueuesState->enterMakeProgress(); }
...
void API::sendv_common(std::vector<IoVec> &ioVec, MP::PayloadDataType dtype,
                       MP::RankID dest, uint32_t tag, MP::Communicator group,
                       CommReq *commReq, int vn) {
  m_dbg.debug(CALL_INFO, 1, 1, "dest=%#x tag=%#x length=%lu vn=%d\n", dest, tag,
              calcLength(ioVec), vn);

  _CommReq *req;
  if (commReq) {
    req = new _CommReq(_CommReq::Isend, ioVec, m_info->sizeofDataType(dtype),
                       dest, tag, group, vn);
    commReq->req = req;
  } else {
    req = new _CommReq(_CommReq::Send, ioVec, m_info->sizeofDataType(dtype),
                       dest, tag, group, vn);
  }
...
void API::send(const Hermes::MemAddr &addr, size_t len, nid_t dest,
               uint64_t tag, int vn) {
...
void API::send(const Hermes::MemAddr &addr, size_t len, MP::RankID dest,
               uint64_t tag, MP::Communicator grp, int vn) {
  std::vector<IoVec> ioVec(1);
  ioVec[0].addr = addr;
  ioVec[0].len = len;
...
void API::isend(void *ptr, size_t len, nid_t dest, uint64_t tag,
                MP::Communicator grp, CommReq *req, int vn) {
  MemAddr addr(1, ptr);
  isend(addr, len, dest, tag, grp, req, vn);
}

void API::isend(const Hermes::MemAddr &addr, size_t len, nid_t dest,
                uint64_t tag, CommReq *req, int vn) {
  std::vector<IoVec> ioVec(1);
  ioVec[0].addr = addr;
  ioVec[0].len = len;
...
void API::isend(const Hermes::MemAddr &addr, size_t len, nid_t dest,
                uint64_t tag, MP::Communicator group, CommReq *req, int vn) {
  std::vector<IoVec> ioVec(1);
  ioVec[0].addr = addr;
  ioVec[0].len = len;
...
void API::recvv_common(std::vector<IoVec> &ioVec, MP::PayloadDataType dtype,
                       MP::RankID src, uint32_t tag, MP::Communicator group,
                       CommReq *commReq) {
  ...
  _CommReq *req;
  if (commReq) {
    req = new _CommReq(_CommReq::Irecv, ioVec, m_info->sizeofDataType(dtype),
                       src, tag, group, 0);
    commReq->req = req;
  } else {
    req = new _CommReq(_CommReq::Recv, ioVec, m_info->sizeofDataType(dtype),
                       src, tag, group, 0);
  }
...
void API::recv(const Hermes::MemAddr &addr, size_t len, nid_t src,
               uint64_t tag) {
  std::vector<IoVec> ioVec(1);
  ioVec[0].addr = addr;
  ioVec[0].len = len;
...
void API::recv(void *ptr, size_t len, nid_t src, uint64_t tag,
...
void API::irecv(const Hermes::MemAddr &addr, size_t len, nid_t src,
                uint64_t tag, CommReq *req) {
  std::vector<IoVec> ioVec(1);
  ioVec[0].addr = addr;
  ioVec[0].len = len;
...
void API::irecv(const Hermes::MemAddr &addr, size_t len, MP::RankID src,
                uint64_t tag, MP::Communicator grp, CommReq *req) {
  std::vector<IoVec> ioVec(1);
  ioVec[0].addr = addr;
  ioVec[0].len = len;
  assert(req);
