...
ProcessQueuesState::ProcessQueuesState(ComponentId_t id, Params &params)
    : SubComponent(id),
      ...
      m_getKey(0), m_rspKey(0), m_needRecv(0), m_numNicRequestedShortBuff(0),
      m_numRecvLooped(0), m_missedInt(false), m_intCtx(NULL), m_simVAddrs(NULL),
      m_numSent(0), m_numRecv(0), m_rendezvousVN(0), m_ackVN(0), m_recvdMsgQ(2),
      m_recvdMsgQpos(0) {
  ...
  m_maxPostedShortBuffers =
      params.find<int32_t>("pqs.maxPostedShortBuffers", 512);
  m_minPostedShortBuffers =
      params.find<int32_t>("pqs.minPostedShortBuffers", 5);
  ...
  m_msgTiming = loadAnonymousSubComponent<MsgTiming>(
      "firefly.msgTiming", "", 0, ComponentInfo::SHARE_NONE, params);
  ...
  m_delayLink = configureSelfLink(
      ...
      new Event::Handler2<ProcessQueuesState,
                          &ProcessQueuesState::delayHandler>(this));

  m_loopLink = configureLink(
      params.find<std::string>("loopBackPortName", "loop"), "1 ns",
      new Event::Handler2<ProcessQueuesState,
                          &ProcessQueuesState::eventLoopHandler>(this));
  assert(m_loopLink);
...
void ProcessQueuesState::setVars(VirtNic *nic, Info *info, MemoryBase *mem,
                                 Thornhill::MemoryHeapLink *memHeapLink,
                                 Link *returnToCaller) {
  m_nic = nic;
  m_info = info;
  m_mem = mem;
...
void ProcessQueuesState::finish() {
  dbg().debug(
      CALL_INFO, 1, 1,
      "pstdRcvQ=%lu recvdMsgQ=%s loopResp=%lu funcStack=%lu sent=%d recv=%d\n",
      m_pstdRcvQ.size(), recvdMsgQsize(), m_loopResp.size(), m_funcStack.size(),
      m_numSent, m_numRecv);
}
...
void ProcessQueuesState::enterInit(bool haveGlobalMemHeap) {
  ...
  size_t length = 0;
  length +=
      (m_maxPostedShortBuffers + m_maxUnexpectedMsg) * (sizeof(MatchHdr) + 16) &
      ~15;
  length +=
      (m_maxPostedShortBuffers + m_maxUnexpectedMsg) * (shortMsgLength() + 16) &
      ~15;
...
void ProcessQueuesState::enterSend(_CommReq *req, uint64_t exitDelay) {
  ...
  if (m_nic->isLocal(calcNid(req, req->getDestRank()), m_nicsPerNode)) {
    callback = std::bind(&ProcessQueuesState::processSendLoop, this, req);
  } else {
    callback = std::bind(&ProcessQueuesState::processSend_0, this, req);
  }
  schedCallback(callback, delay);
}

void ProcessQueuesState::processSend_0(_CommReq *req) {
  m_mem->write(std::bind(&ProcessQueuesState::processSend_1, this, req), 0,
               sizeof(req->hdr()));
}

void ProcessQueuesState::processSend_1(_CommReq *req) {
  VoidFunction callback =
      std::bind(&ProcessQueuesState::processSend_2, this, req);
...
void ProcessQueuesState::processSend_2(_CommReq *req) {
  ...
  if (length <= shortMsgLength()) {
    ...
    *callback = std::bind(&ProcessQueuesState::dmaRecvFiniGI, this, info,
                          hdrVec[0].addr.getSimVAddr(), std::placeholders::_1,
  ...
  *callback = std::bind(&ProcessQueuesState::pioSendFiniVoid, this, ptrs,
                        hdrVec.addr.getSimVAddr());

  m_nic->pioSend(vn, nid, ShortMsgQ, vec, callback, req->route_id());
...
void ProcessQueuesState::processSendLoop(_CommReq *req) {
  ...
  std::vector<IoVec> vec;
  vec.insert(vec.begin(), hdrVec);
  vec.insert(vec.begin() + 1, req->ioVec().begin(), req->ioVec().end());

  dbg().debug(
      CALL_INFO, 2, DBG_MSK_PQS_APP_SIDE, "destCore=%d\n",
      m_nic->calcCoreId(calcNid(req, req->getDestRank()), m_nicsPerNode));
  loopSendReq(
      vec, m_nic->calcCoreId(calcNid(req, req->getDestRank()), m_nicsPerNode),
      req);
...
void ProcessQueuesState::enterRecv(_CommReq *req, uint64_t exitDelay) {
  ...
  if (length > shortMsgLength()) {
    callback = std::bind(&ProcessQueuesState::processRecv_0, this, req);
  } else {
    callback = std::bind(&ProcessQueuesState::processRecv_1, this, req);
  }
...
void ProcessQueuesState::processRecv_0(_CommReq *req) {
  ...
  m_mem->pin(std::bind(&ProcessQueuesState::processRecv_1, this, req), 0,
             req->getLength());
}
...
void ProcessQueuesState::processRecv_2(Stack *stack, _CommReq *req) {
  ...
  if (!m_pstdRcvPreQ.empty()) {
    dbg().debug(CALL_INFO, 2, DBG_MSK_PQS_APP_SIDE,
                "no match against unexpected queue move to pstRecvQ\n");
...
void ProcessQueuesState::enterMakeProgress(uint64_t exitDelay) {
  ...
  if (m_recvdMsgQ[m_recvdMsgQpos].empty()) {
    ...
    WaitCtx *ctx =
        new WaitCtx(NULL, std::bind(&ProcessQueuesState::processMakeProgress,
                                    this, &m_funcStack));
...
void ProcessQueuesState::enterCancel(MP::MessageRequest req,
                                     uint64_t exitDelay) {
...
void ProcessQueuesState::enterTest(WaitReq *req, int *flag,
                                   uint64_t exitDelay) {
  dbg().debug(CALL_INFO, 1, DBG_MSK_PQS_APP_SIDE, "\n");
  ...
  WaitCtx *ctx = new WaitCtx(
      req, std::bind(&ProcessQueuesState::processWait_0, this, &m_funcStack),
      flag);
...
void ProcessQueuesState::processWait_0(Stack *stack) {
  dbg().debug(CALL_INFO, 2, DBG_MSK_PQS_APP_SIDE,
              "stack.size()=%lu num pstd %lu, recvdMsgQ %s\n", stack->size(),
              m_pstdRcvQ.size(), recvdMsgQsize());
  ...
  if (req) {
    ...
    // WaitCtx is blocked
    dbg().debug(CALL_INFO, 2, DBG_MSK_PQS_APP_SIDE,
                "blocking call enableInt() return to processWait_0()\n");
    enableInt(ctx, &ProcessQueuesState::processWait_0);
  }
}
...
void ProcessQueuesState::processWaitCtx_1(WaitCtx *ctx, _CommReq *req) {
  ...
  if (length > shortMsgLength()) {
    ...
    m_mem->unpin(std::bind(&ProcessQueuesState::processWaitCtx_2, this, ctx), 0,
                 length);
  } else {
...
void ProcessQueuesState::processQueues(Stack *stack) {
  dbg().debug(
      CALL_INFO, 1, DBG_MSK_PQS_Q,
      "recvdMsgQ=%s m_unexpectedMsgQ=%zu m_pstdRecvPre=%zu m_pstdRcvQ=%zu\n",
      recvdMsgQsize(), m_unexpectedMsgQ.size(), m_pstdRcvPreQ.size(),
      m_pstdRcvQ.size());
  ...
  while (m_needRecv && m_numNicRequestedShortBuff < m_maxUnexpectedMsg) {
    dbg().debug(CALL_INFO, 1, DBG_MSK_PQS_Q,
                "post buff for needRecv %d m_numNicRequestedShortBuff=%d\n",
...
void ProcessQueuesState::processShortList_1(Stack *stack) {
  ...
  m_mem->walk(std::bind(&ProcessQueuesState::processShortList_2, this, stack),
              count);
}
...
void ProcessQueuesState::processShortList_2(Stack *stack) {
  ...
  if (ctx->req) {
    schedCallback(
        std::bind(&ProcessQueuesState::processShortList_3, this, stack),
...
void ProcessQueuesState::processShortList_3(Stack *stack) {
  ...
  if (length <= shortMsgLength() || dynamic_cast<LoopReq *>(ctx->msg())) {
    dbg().debug(CALL_INFO, 2, DBG_MSK_PQS_Q,
                "copyIoVec() short|loop message\n");

    copyIoVec(req->ioVec(), ctx->ioVec(), length);

    m_mem->copy(std::bind(&ProcessQueuesState::processShortList_4, this, stack),
                1, 0, length);
...
void ProcessQueuesState::dmaRecvFiniGI(GetInfo *info, uint64_t simVAddr,
                                       nid_t nid, uint32_t tag, size_t length) {
...
void ProcessQueuesState::dmaRecvFiniSRB(ShortRecvBuffer *buf, nid_t nid,
  ...
  dbg().debug(CALL_INFO, 2, DBG_MSK_PQS_CB,
              "ShortMsgQ nid=%#x tag=%#x length=%lu\n", nid, tag, length);
  dbg().debug(CALL_INFO, 2, DBG_MSK_PQS_CB,
              "ShortMsgQ rank=%d tag=%#" PRIx64 " count=%d "
              "dtypeSize=%d\n",
              buf->hdr.rank, buf->hdr.tag, buf->hdr.count, buf->hdr.dtypeSize);
...
void ProcessQueuesState::enableInt(
    FuncCtxBase *ctx, void (ProcessQueuesState::*funcPtr)(Stack *)) {
...
void ProcessQueuesState::pioSendFiniVoid(std::vector<void *> ptrs,
                                         uint64_t simVAddr) {
...
void ProcessQueuesState::processLongGetFini0(Stack *stack) {
  ...
  VoidFunction *callback = new VoidFunction;
  *callback = std::bind(&ProcessQueuesState::pioSendFiniCtrlHdr, this, hdr,
                        hdrVec.addr.getSimVAddr());
  ...
  dbg().debug(CALL_INFO, 1, DBG_MSK_PQS_CB,
              "send long msg Ack to nid=%d key=%#x\n", req->m_ackNid,
              req->m_ackKey);
...
void ProcessQueuesState::loopHandler(int srcCore, std::vector<IoVec> &vec,
                                     void *key) {

  MatchHdr *hdr = (MatchHdr *)vec[0].addr.getBacking();

  dbg().debug(CALL_INFO, 2, DBG_MSK_PQS_CB,
              "req: srcCore=%d key=%p vec.size()=%lu srcRank=%d\n", srcCore,
              key, vec.size(), hdr->rank);
...
_CommReq *ProcessQueuesState::searchPostedRecv(std::deque<_CommReq *> &pstd,
                                               MatchHdr &hdr, int &count) {
  _CommReq *req = NULL;
...
bool ProcessQueuesState::checkMatchHdr(MatchHdr &hdr, MatchHdr &wantHdr,
                                       uint64_t ignore) {
  dbg().debug(CALL_INFO, 2, DBG_MSK_PQS_Q,
              "posted tag %#" PRIx64 ", msg tag %#" PRIx64 "\n", wantHdr.tag,
              hdr.tag);
  ...
  dbg().debug(CALL_INFO, 2, DBG_MSK_PQS_Q, "want dtypeSize %d %d\n",
              wantHdr.dtypeSize, hdr.dtypeSize);
...
void ProcessQueuesState::copyIoVec(std::vector<IoVec> &dst,
                                   std::vector<IoVec> &src, size_t len) {
  dbg().debug(CALL_INFO, 2, DBG_MSK_PQS_Q,
              "dst.size()=%lu src.size()=%lu wantLen=%lu\n", dst.size(),
              src.size(), len);
...
void ProcessQueuesState::postShortRecvBuffer() {
  ...
  Callback2 *callback = new Callback2;
  *callback = std::bind(&ProcessQueuesState::dmaRecvFiniSRB, this, buf,
  ...
  dbg().debug(CALL_INFO, 2, DBG_MSK_PQS_POST_SHORT,
              "num postedShortRecvBuffers %lu\n", m_postedShortBuffers.size());
  m_nic->dmaRecv(-1, ShortMsgQ, buf->ioVec, callback);
}

void ProcessQueuesState::loopSendReq(std::vector<IoVec> &vec, int core,
                                     void *key) {
...
void ProcessQueuesState::eventLoopHandler(Event *ev) {
  ...
  m_dbg.debug(CALL_INFO, 1, DBG_MSK_PQS_LOOP, "%s core=%d key=%p\n",
              event->vec.empty() ? "Response" : "Request", event->core,
              event->key);
