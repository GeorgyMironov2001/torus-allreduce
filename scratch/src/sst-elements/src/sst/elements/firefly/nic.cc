...
Nic::Nic(ComponentId_t id, Params &params)
    : Component(id), m_detailedCompute(2, NULL), m_useDetailedCompute(false),
      m_getKey(10), m_memoryModel(NULL), m_respKey(1), m_predNetIdleTime(0),
      m_linkBytesPerSec(0), m_detailedInterface(NULL), m_getHdrVN(0),
      m_getRespLargeVN(0), m_getRespSmallVN(0) {
  ...
  bool printConfig =
      (0 == params.find<std::string>("printConfig", "no").compare("yes"));

  // The link between the NIC and HOST historically provided the latency of
  // crossing a bus such as PCI hence it was configured at wire up with a value
  // like 150ns. The NIC has since taken on some HOST functionality so the
  // latency has been dropped to 1ns. The bus latency must still be added for
  // some messages so the NIC now sends these message to itself with a time of
  // nic2host_lat_ns - "the latency of the link".
  ...
  m_unitPool = new UnitPool(
      m_dbg, params.find<std::string>("nicAllocationPolicy", "RoundRobin"),
      numRecvNicUnits, numSendMachines, 1, m_num_vNics);
  ...
  m_linkControl = loadUserSubComponent<Interfaces::SimpleNetwork>(
      "rtrLink", ComponentInfo::SHARE_NONE, m_numVN);
  ...
  m_linkRecvWidget = new LinkControlWidget(
      m_dbg,
      [=]() {
  ...
  m_linkSendWidget = new LinkControlWidget(
      m_dbg,
      [=]() {
  ...
  m_selfLink =
      configureSelfLink("Nic::selfLink", "1 ns",
  ...
  for (int i = 0; i < m_num_vNics; i++) {
    m_vNicV.push_back(
        new VirtNic(this, i, params.find<std::string>("corePortName", "core"),
                    getDelay_ns()));
  }

  Params shmemParams = params.get_scoped_params("shmem");
  m_shmem = new Shmem(*this, shmemParams, m_myNodeId, m_num_vNics, m_dbg,
                      getDelay_ns(), getDelay_ns());
  size_t FAM_memSizeBytes =
      params.find<SST::UnitAlgebra>("FAM_memSize").getRoundedValue();
  if (FAM_memSizeBytes) {
    if (printConfig) {
  ...
  if (params.find<int>("useSimpleMemoryModel", 0)) {
    ...
    std::string useDetailed =
        smmParams.find<std::string>("useDetailedModel", "no");
    ...
    if (isdigit(useDetailed[0])) {
      if (findNid(m_myNodeId, useDetailed)) {
        m_detailedInterface = loadUserSubComponent<DetailedInterface>(
            "detailedInterface", ComponentInfo::SHARE_STATS);
    ...
    m_memoryModel = loadAnonymousSubComponent<MemoryModel>(
        "firefly.SimpleMemory", "", 0,
        ComponentInfo::SHARE_STATS | ComponentInfo::INSERT_STATS, smmParams);

    if (m_detailedInterface) {
      static_cast<SimpleMemoryModel *>(m_memoryModel)
          ->setDetailedInterface(m_detailedInterface);
    }
  }
  if (params.find<bool>("useTrivialMemoryModel", false)) {
    if (m_memoryModel) {
      m_dbg.fatal(
          CALL_INFO, 0,
          "can't use TrivialMemoryModel, memoryModel already configured\n");
    }
    Params smmParams = params.get_scoped_params("simpleMemoryModel");
  ...
  for (int i = 0; i < m_numVN; i++) {
    m_recvMachine.push_back(new RecvMachine(
        *this, i, m_vNicV.size(), m_myNodeId,
        params.find<uint32_t>("verboseLevel", 0),
        params.find<uint32_t>("verboseMask", -1), rxMatchDelay, hostReadDelay,
        maxRecvMachineQsize, params.find<int>("maxActiveRecvStreams", 16),
  ...
  for (int i = 0; i < numSendMachines - 1; i++) {
    SendMachine *sm = new SendMachine(
        *this, m_myNodeId, params.find<uint32_t>("verboseLevel", 0),
        params.find<uint32_t>("verboseMask", -1), i, packetSizeInBytes,
        packetOverhead, maxSendMachineQsize, allocNicSendUnit(), false);
    m_sendMachineQ.push(sm);
    m_sendMachineV[i] = sm;
  }

  m_sendMachineV[numSendMachines - 1] = new SendMachine(
      *this, m_myNodeId, params.find<uint32_t>("verboseLevel", 0),
      params.find<uint32_t>("verboseMask", -1), m_sendMachineV.size() - 1,
      packetSizeInBytes, packetOverhead, maxSendMachineQsize,
      allocNicSendUnit(), true);

  float dmaBW = params.find<float>("dmaBW_GBs", 0.0);
  float dmaContentionMult = params.find<float>("dmaContentionMult", 0.0);
  m_arbitrateDMA =
      new ArbitrateDMA(*this, m_dbg, dmaBW, dmaContentionMult, 100000);
  ...
  if (!params.find<int>("useSimpleMemoryModel", 0) && !dtldName.empty()) {
    ...
    detailed =
        loadUserSubComponent<Thornhill::DetailedCompute>("nicDetailedRead");
    ...
    detailed =
        loadUserSubComponent<Thornhill::DetailedCompute>("nicDetailedWrite");
    if (detailed && detailed->isConnected()) {
...
Nic::~Nic() {
  ...
  if (m_recvNotifyFunctor)
    delete m_recvNotifyFunctor;
  if (m_sendNotifyFunctor)
    delete m_sendNotifyFunctor;

  if (m_detailedCompute[0])
    delete m_detailedCompute[0];
  if (m_detailedCompute[1])
    delete m_detailedCompute[1];
...
void Nic::dmaSend(NicCmdEvent *e, int vNicNum) {
  std::function<void(void *)> callback =
      std::bind(&Nic::notifySendDmaDone, this, vNicNum, _1);

  CmdSendEntry *entry =
      new CmdSendEntry(vNicNum, getSendStreamNum(vNicNum), e, callback);
...
void Nic::pioSend(NicCmdEvent *e, int vNicNum) {
  std::function<void(void *)> callback =
      std::bind(&Nic::notifySendPioDone, this, vNicNum, _1);

  CmdSendEntry *entry =
      new CmdSendEntry(vNicNum, getSendStreamNum(vNicNum), e, callback);

  m_dbg.debug(CALL_INFO, 1, 1,
              "src_vNic=%d dest=%#x dst_vNic=%d tag=%#x "
              "vecLen=%lu totalBytes=%lu vn=%d\n",
              vNicNum, e->node, e->dst_vNic, e->tag, e->iovec.size(),
              entry->totalBytes(), e->vn);

  qSendEntry(entry);
}

void Nic::dmaRecv(NicCmdEvent *e, int vNicNum) {
  DmaRecvEntry::Callback callback =
      std::bind(&Nic::notifyRecvDmaDone, this, vNicNum, _1, _2, _3, _4, _5);
...
void Nic::get(NicCmdEvent *e, int vNicNum) {
  ...
  qSendEntry(new GetOrgnEntry(vNicNum, getSendStreamNum(vNicNum), e->node,
                              e->dst_vNic, e->tag, getKey, m_getHdrVN));
}
...
void Nic::put(NicCmdEvent *e, int vNicNum) {
  ...
  std::function<void(void *)> callback =
      std::bind(&Nic::notifyPutDone, this, vNicNum, _1);
  CmdSendEntry *entry =
      new CmdSendEntry(vNicNum, getSendStreamNum(vNicNum), e, callback);
  m_dbg.debug(CALL_INFO, 1, 1,
              "src_vNic=%d dest=%#x dst_vNic=%d tag=%#x "
...
void Nic::qSendEntry(SendEntryBase *entry) {
  ...
  m_dbg.debug(CALL_INFO, 2, NIC_DBG_SEND_MACHINE,
              "myPid=%d destNode=%d destPid=%d size=%zu %s\n",
              entry->local_vNic(), entry->dest(), entry->dst_vNic(),
              entry->totalBytes(),
              entry->isCtrl()  ? "Ctrl"
              : entry->isAck() ? "Ack"
                               : "Std");
  ...
  if (entry->totalBytes() >= MaxPayload) {
    m_dbg.fatal(CALL_INFO, -1,
                "Error: maximum network message size must be less than %d "
                "bytes, requested size %zu\n",
...
void Nic::feedTheNetwork(int vn) {
  ...
  while (!pq.empty()) {
    ...
    if (!ret) {
      ...
      SimTime_t curTime = getCurrentSimCycle();
      SimTime_t latPS =
          ((double)x.pkt->payloadSize() / (double)m_linkBytesPerSec) *
          1000000000000;
...
void Nic::sendPkt(FireflyNetworkEvent *ev, int dest, int vn) {
  ...
  req->vn = vn;
  req->setRoute_id(ev->getRoute_id());
  req->givePayload(ev);
  if ((m_tracedPkt == m_packetId || m_tracedPkt == -2) &&
      m_tracedNode == getNodeId()) {
  ...
  m_dbg.debug(CALL_INFO, 3, NIC_DBG_SEND_NETWORK,
              ...
              req->dest, ev->bufSize(), (uint64_t)m_packetId,
              ev->isHdr() ? "Hdr" : "", ev->isTail() ? "Tail" : "");
...
void Nic::detailedMemOp(Thornhill::DetailedCompute *detailed,
                        std::vector<MemOp> &vec, std::string op,
                        Callback callback) {

  std::deque<std::pair<std::string, SST::Params>> gens;
  m_dbg.debug(CALL_INFO, 1, NIC_DBG_DETAILED_MEM, "%s %zu vectors\n",
              op.c_str(), vec.size());
...
void Nic::dmaRead(int unit, int pid, std::vector<MemOp> *vec,
                  Callback callback) {
...
void Nic::dmaWrite(int unit, int pid, std::vector<MemOp> *vec,
                   Callback callback) {
...
Hermes::MemAddr Nic::findShmem(int core, Hermes::Vaddr addr, size_t length) {
  ...
  m_dbg.debug(CALL_INFO, 1, NIC_DBG_SHMEM,
              "found region core=%d Vaddr=%#" PRIx64 " length=%lu\n", core,
  ...
  if (addr + length > region.first.getSimVAddr() + region.second) {
    m_dbg.fatal(CALL_INFO, -1,
                "address %#" PRIx64 " not in region %#" PRIx64 " length %lu \n",
