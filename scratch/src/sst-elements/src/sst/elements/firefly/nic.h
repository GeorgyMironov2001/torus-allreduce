...
public:
  SST_ELI_REGISTER_COMPONENT(Nic, "firefly", "nic",
  ...
  SST_ELI_DOCUMENT_PARAMS(
      ...
      {"maxSendMachineQsize", "Sets the number of pending memory operations",
       "1"},
      {"maxRecvMachineQsize", "Sets the number of pending memory operations",
       "1"},
      ...
      {"FAM_memsize", "", "0"},
      {"FAM_backed", "Controls whether FAM memory is backed in the simlation",
       "yes"},

      {"useSimpleMemoryModel", "If set to 1 use the simple memory model", "0"},
      {"useTrivialMemoryModel", "Use the trivial memory model", "false"},

      {"maxActiveRecvStreams", "Set max number of active receive streams",
       "16"},
  ...
  */
  ...
  SST_ELI_DOCUMENT_STATISTICS(
      ...
      {"networkStall",
       "number of picoseconds the outbound network port was blocked", "latency",
       1},
      {"hostStall",
       "number of nanoseconds the host blocked inbound network packets",
       "latency", 1},

      {"recvStreamPending",
       "number of pending receive stream memory operations", "depth", 1},
      {"sendStreamPending", "number of pending send stream memory operations",
       "depth", 1},

      {"detailed_num_reads", "total number of loads", "count", 1},
      {"detailed_num_writes", "total number of stores", "count", 1},
      {"detailed_req_latency", "Running total of all latency for all requests",
       "count", 1}, )

  SST_ELI_DOCUMENT_PORTS({"rtr", "Port connected to the router", {}},
                         {"read", "Port connected to the detailed model", {}},
                         {"write", "Port connected to the detailed model", {}},
                         {"core%(num_vNics)d",
                          "Ports connected to the network driver",
                          {}},
                         {"nicDetailedRead",
                          "Port connected to the detailed model",
...
private:
  ...
  public:
    LinkControlWidget(Output &output, Callback callback, int numVN)
        : m_dbg(output), m_notifiers(numVN, NULL), m_num(numVN, 0),
          m_callback(callback) {}
...
private:
  ...
  public:
    ...
    SelfEvent(Callback_t callback) : type(Callback), callback(callback) {}
  ...
  class MemRgnEntry {
  public:
    MemRgnEntry(std::vector<IoVec> &iovec) : m_iovec(iovec) {}
  ...
  template <class T> class PriorityEntry : public Priority {
  public:
    PriorityEntry(SimTime_t p1, int p2, T data)
        : Priority(p1, p2), m_data(data) {}
...
public:
  ...
  VirtNic *getVirtNic(int id) { return m_vNicV[id]; }

  void shmemDecPendingPuts(int core) { m_shmem->decPendingPuts(core); }

  std::vector<RecvCtxData> m_recvCtxData;

private:
  typedef uint64_t DestKey;
  static DestKey getDestKey(int node, int pid) {
    return (DestKey)node << 32 | pid;
  }
  ...
  SimTime_t getShmemRxDelay_ns() { return m_shmemRxDelay_ns; }
  ...
  void notifyRecvDmaDone(int vNic, int src_vNic, int src, int tag, size_t len,
                         void *key) {
  ...
  uint16_t genGetKey() { return ++m_getKey; }
  ...
  struct X {
    X(Callback callback, FireflyNetworkEvent *pkt, int dest)
        : callback(callback), pkt(pkt), dest(dest) {}
  ...
  int getSendStreamNum(int pid) {
    ...
    m_dbg.debug(CALL_INFO, 3, NIC_DBG_SEND_MACHINE,
                "pid=%d stream=%d next=%d\n", pid, val, m_sendStreamNum[pid]);
    return val;
  }
  ...
  int allocNicAckUnit() { return m_unitPool->allocAckUnit(); }

  int allocNicSendUnit() { return m_unitPool->allocSendUnit(); }

  int allocNicRecvUnit(int pid) { return m_unitPool->allocRecvUnit(pid); }
