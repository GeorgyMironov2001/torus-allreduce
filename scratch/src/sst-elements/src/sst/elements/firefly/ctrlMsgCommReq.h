...
public:
  ...
  _CommReq(Type type, std::vector<IoVec> &_ioVec, unsigned int dtypeSize,
           MP::RankID rank, uint32_t tag, MP::Communicator group, int vn,
           int route_id = -1)
      : m_type(type), m_ioVec(_ioVec), m_resp(NULL), m_done(false),
        m_destRank(MP::AnySrc), m_ignore(0), m_isMine(true), m_finiDelay_ns(0),
        m_vn(vn), m_route_id(route_id) {
  ...
  _CommReq(Type type, const Hermes::MemAddr &buf, uint32_t count,
           ...
           int route_id = -1)
      : m_type(type), m_resp(resp), m_done(false), m_destRank(MP::AnySrc),
        m_ignore(0), m_isMine(true), m_finiDelay_ns(0), m_vn(vn),
        m_route_id(route_id) {
    m_hdr.count = count;
    m_hdr.dtypeSize = dtypeSize;
  ...
  bool isBlocking() { return m_type == Recv || m_type == Send; }

  uint64_t ignore() { return m_ignore; }
  void setSrcRank(MP::RankID rank) { m_hdr.rank = rank; }
  ...
  MP::MessageResponse *getResp() { return &m_matchInfo; }
  ...
  int route_id() { return m_route_id; }
