...
static std::string getLogicalGroupParam(const Params &params, Topology *topo,
                                        int port, std::string param,
  ...
  if (value == "") {
    ...
    if (value == "") {
      ...
      merlin_abort.fatal(CALL_INFO, -1,
                         "hr_router requires %s to be specified\n",
                         param.c_str());
    }
  }
  return value;
...
static UnitAlgebra getLogicalGroupParamUA(const Params &params, Topology *topo,
                                          int port, std::string param,
                                          std::string default_val = "") {

  std::string value =
      getLogicalGroupParam(params, topo, port, param, default_val);
...
hr_router::hr_router(ComponentId_t cid, Params &params)
    : Router(cid), num_vcs(-1), output(getSimulationOutput()) {

  // Get the options for the router
  id = params.find<int>("id", -1);
  if (id == -1) {
    merlin_abort.fatal(CALL_INFO, -1,
                       "hr_router requires id to be specified\n");
  }

  num_ports = params.find<int>("num_ports", -1);
  if (num_ports == -1) {
    merlin_abort.fatal(CALL_INFO, -1,
                       "hr_router requires num_ports to be specified\n");
  }
  ...
  if (!topo) {
    merlin_abort.fatal(
        CALL_INFO_LONG, 1,
        "hr_router requires topology to be specified in input file\n");
  }

  topo->getVCsPerVN(vcs_per_vn);
  num_vcs = 0;
  for (int vcs : vcs_per_vn)
    num_vcs += vcs;
  ...
  if (vn_remap_shm != "") {
    ...
    if (vec.size() == 0) {
      merlin_abort.fatal(CALL_INFO, 1,
                         "if vn_remap_shm is specified, a map must be supplied "
                         "using vn_remap\n");
    }
  ...
  if (flit_size_s == "") {
    merlin_abort.fatal(CALL_INFO, -1,
                       "hr_router requires flit_size to be specified\n");
    abort();
  }
  ...
  if (xbar_bw_s == "") {
    merlin_abort.fatal(CALL_INFO, -1,
                       "hr_router requires xbar_bw to be specified\n");
  }
  ...
  std::string output_latency =
      params.find<std::string>("output_latency", "0ns");
  ...
  std::string output_buf_size =
      params.find<std::string>("output_buf_size", "0");
  ...
  std::string inspector_config =
      params.find<std::string>("network_inspectors", "");
  ...
  if (pc_params.contains("network_inspectors"))
    pc_params.insert("network_inspectors",
                     params.find<std::string>("network_inspectors", ""));
  pc_params.insert("oql_track_port",
                   params.find<std::string>("oql_track_port", "false"));
  pc_params.insert("oql_track_remote",
                   params.find<std::string>("oql_track_remote", "false"));
  ...
  for (int i = 0; i < num_ports; i++) {
    ...
    pc_params.insert(
        "input_latency",
        getLogicalGroupParam(params, topo, i, "input_latency", "0ns"));
    pc_params.insert(
        "output_latency",
        getLogicalGroupParam(params, topo, i, "output_latency", "0ns"));
    pc_params.insert("input_buf_size",
                     getLogicalGroupParam(params, topo, i, "input_buf_size"));
    pc_params.insert("output_buf_size",
  ...
  std::string xbar_arb =
      params.find<std::string>("xbar_arb", "merlin.xbar_arb_lru");
  ...
  my_clock_handler =
      new Clock::Handler2<hr_router, &hr_router::clock_handler>(this);
  ...
  if (xbar_bw_ua < link_bw) {
    merlin_abort.fatal(CALL_INFO_LONG, 1,
                       "ERROR: hr_router requires xbar_bw to be greater than "
                       "or equal to link_bw\n"
                       "  xbar_bw = %s, link_bw = %s\n",
  ...
  for (int i = 0; i < num_ports; i++) {
    ...
    if (progress_vcs[i] > -1) {
      ...
      if (ev->getTraceType() == SimpleNetwork::Request::FULL) {
        output.output(
            "TRACE(%d): %" PRIu64 " ns: Copying event (src = %d, dest = %d) "
...
void hr_router::init(unsigned int phase) {
  for (int i = 0; i < num_ports; i++) {
    ...
    while ((ev = ports[i]->recvUntimedData()) != NULL) {
      ...
      for (std::vector<int>::iterator j = outPorts.begin(); j != outPorts.end();
           ++j) {
...
void hr_router::sendCtrlEvent(CtrlRtrEvent *ev, int port) {
  if (port == -1) {
...
void hr_router::recvCtrlEvent(int port, CtrlRtrEvent *ev) {
  ...
  default:
    ...
    if (port == -1) {
      ...
      if (dest.addr_is_router) {
        ...
        // it
        fatal(CALL_INFO_LONG, -1,
              "ERROR: router %d received unknown ctrl event\n", id);
      } else {
...
void hr_router::init_vcs() {
  ...
  for (int i = 0; i < num_ports; i++) {
    ports[i]->initVCs(num_vns, vcs_per_vn.data(), &vc_heads[i * num_vcs],
...
void hr_router::reportIncomingEvent(internal_router_event *ev) {
