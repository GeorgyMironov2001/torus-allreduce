...
topo_torus::topo_torus(ComponentId_t cid, Params &params, int num_ports,
...
void topo_torus::route_packet(int port, int vc, internal_router_event *ev) {
    ...
    for (int dim = tt_ev->routing_dim; dim < dimensions; dim++) {
      if (tt_ev->dest_loc[dim] != id_loc[dim]) {
        ...
        output.verbose(CALL_INFO, 1, 1,
                       " %d to %d:  Dist Neg: %d, Dist Pos: %d\n", id_loc[dim],
                       tt_ev->dest_loc[dim], dist_neg, dist_pos);

        int p =
            choose_multipath(port_start[dim][(go_pos) ? 0 : 1], dim_width[dim],
...
void topo_torus::routeUntimedData(int port, internal_router_event *ev,
...
std::string topo_torus::getPortLogicalGroup(int port) const
{
...
int topo_torus::get_dest_router(int dest_id) const {
  return dest_id / num_local_ports;
}

int topo_torus::get_dest_local_port(int dest_id) const {
