...
class topoSimple(Topo):
    def __init__(self):
        Topo.__init__(self)
        self.topoKeys.extend(["topology", "debug", "num_ports", "flit_size", "link_bw",
                             "xbar_bw", "input_latency", "output_latency", "input_buf_size", "output_buf_size"])
        self.topoOptKeys.extend(["xbar_arb", "num_vns", "vn_remap", "vn_remap_shm", "portcontrol.output_arb",
...
class topoTorus(Topo):
    def __init__(self):
        Topo.__init__(self)
        self.topoKeys.extend(["topology", "debug", "num_ports", "flit_size", "link_bw", "xbar_bw", "torus.shape",
                             "torus.width", "torus.local_ports", "input_latency", "output_latency", "input_buf_size", "output_buf_size"])
        self.topoOptKeys.extend(["xbar_arb", "num_vns", "vn_remap", "vn_remap_shm", "portcontrol.output_arb", "portcontrol.arbitration.qos_settings",
                                "portcontrol.arbitration.arb_vns", "portcontrol.arbitration.arb_vcs", "link_bw:host", "link_bw:torus"])
        self.params = None
    ...
    def prepParams(self):
        ...
        if not "torus.width" in _params:
            ...
            self.dimwidths = [int(x)
                              for x in _params["torus.width"].split('x')]
    ...
    def build(self):
        ...
        swap_keys = [("torus.shape", "shape"), ("torus.width", "width"), (
            "torus.local_ports", "local_ports"), ("torus.route_table_file", "route_table_file")]
        ...
        for i in range(num_routers):
            ...
            rtr.addParams(_params.subset(self.topoKeys, self.topoOptKeys))
            rtr.addParam("id", i)
            topology_name = "merlin.torus"
            if "route_table_file" in _topo_params:
                topology_name = "merlin.torus_trees"
            topology = rtr.setSubComponent("topology", topology_name)
            ...
            for dim in range(self.nd):
                ...
                for num in range(self.dimwidths[dim]):
                    rtr.addLink(getLink(mylocstr, theirlocstr, num),
                                "port%d" % port, _params["link_lat"])
                    port = port+1

                # Negative direction
                theirdims[dim] = ((mydims[dim] - 1) +
                                  self.dims[dim]) % self.dims[dim]
                theirlocstr = self._formatShape(theirdims)
                for num in range(self.dimwidths[dim]):
                    rtr.addLink(getLink(theirlocstr, mylocstr, num),
                                "port%d" % port, _params["link_lat"])
                    port = port+1
            ...
            for n in range(_params["torus.local_ports"]):
                ...
                if ep:
                    ...
                    if self.bundleEndpoints:
                        nicLink.setNoCut()
                    nicLink.connect(ep, (rtr, "port%d" %
                                    port, _params["link_lat"]))
                port = port+1
...
class topoMesh(Topo):
    def __init__(self):
        ...
        self.topoKeys = ["topology", "debug", "num_ports", "flit_size", "link_bw", "xbar_bw", "mesh.shape",
                         "mesh.width", "mesh.local_ports", "input_latency", "output_latency", "input_buf_size", "output_buf_size"]
    ...
    def build(self):
        ...
        swap_keys = [("mesh.shape", "shape"), ("mesh.width",
                                               "width"), ("mesh.local_ports", "local_ports")]
...
class topoHyperX(Topo):
    ...
    def prepParams(self):
        ...
        if not "hyperx.width" in _params:
            ...
            self.dimwidths = [int(x)
                              for x in _params["hyperx.width"].split('x')]
    ...
    def build(self):
        ...
        swap_keys = [("hyperx.shape", "shape"), ("hyperx.width", "width"),
                     ("hyperx.local_ports", "local_ports"), ("hyperx.algorithm", "algorithm")]
...
#        print("Total hosts: " + str(self.total_hosts))
        ...
        for i in range(1, len(self.downs)):
            self.routers_per_level[i] = self.routers_per_level[i -
                                                               1] * self.ups[i-1] // self.downs[i]

        self.start_ids = [0] * len(self.downs)
        for i in range(1, len(self.downs)):
            self.start_ids[i] = self.start_ids[i-1] + \
                self.routers_per_level[i-1]
        ...
        for i in range(1, len(self.downs)-1):
            self.groups_per_level[i] = self.groups_per_level[i -
                                                             1] // self.downs[i]
    ...
    #    print("start id: " + str(id))
        ...
        if level == 0:
            ...
            for l in range(len(links)):
                rtr.addLink(links[l], "port%d" %
                            (l+self.downs[0]), _params["link_lat"])
            return
    ...
    def build(self):
        ...
        swap_keys = [("fattree.shape", "shape"), ("fattree.algorithm",
                                                  "algorithm"), ("fattree.adaptive_threshold", "adaptive_threshold")]
        ...
        if self.ups:  # True for all cases except for single level
            ...
            for i in range(rtrs_in_group):
                for j in range(self.downs[level]):
                    ...
                    rtr_links[i].append(
                        sst.Link("link_l%d_g0_r%d_p%d" % (level, i, j)))
            ...
            for i in range(self.routers_per_level[level]):
                ...
                for l in range(len(rtr_links[i])):
                    rtr.addLink(rtr_links[i][l], "port%d" %
                                l, _params["link_lat"])
...
class topoDragonFly(Topo):
    def __init__(self):
        Topo.__init__(self)
        self.topoKeys = ["topology", "debug", "num_ports", "flit_size", "link_bw", "xbar_bw", "dragonfly.hosts_per_router", "dragonfly.routers_per_group", "dragonfly.intergroup_per_router",
                         "dragonfly.num_groups", "dragonfly.intergroup_links", "input_latency", "output_latency", "input_buf_size", "output_buf_size", "dragonfly.global_route_mode"]
        self.topoOptKeys = ["xbar_arb", "link_bw.host", "link_bw.group", "link_bw.global", "input_latency.host", "input_latency.group", "input_latency.global", "output_latency.host", "output_latency.group", "output_latency.global", "input_buf_size.host", "input_buf_size.group",
                            "input_buf_size.global", "output_buf_size.host", "output_buf_size.group", "output_buf_size.global", "num_vns", "vn_remap", "vn_remap_shm", "portcontrol.output_arb", "portcontrol.arbitration.qos_settings", "portcontrol.arbitration.arb_vns", "portcontrol.arbitration.arb_vcs"]
...
#        _params["router_radix"] = int(_params["router_radix"])
        ...
        _params["num_peers"] = _params["dragonfly.hosts_per_router"] * \
            _params["dragonfly.routers_per_group"] * \
            _params["dragonfly.num_groups"]
        _params["dragonfly.global_route_mode"] = self.global_routes

        self.total_intergroup_links = (
            _params["dragonfly.num_groups"] - 1) * _params["dragonfly.intergroup_links"]
        _params["dragonfly.intergroup_per_router"] = int(
            (self.total_intergroup_links + _params["dragonfly.routers_per_group"] - 1) // _params["dragonfly.routers_per_group"])
        self.empty_ports = _params["dragonfly.intergroup_per_router"] * \
            _params["dragonfly.routers_per_group"] - \
            self.total_intergroup_links

        _params["router_radix"] = _params["dragonfly.routers_per_group"]-1 + \
            _params["dragonfly.hosts_per_router"] + \
    ...
    def build(self):
        ...
        swap_keys = [("dragonfly.hosts_per_router", "hosts_per_router"), ("dragonfly.routers_per_group", "routers_per_group"), ("dragonfly.intergroup_links", "intergroup_links"), ("dragonfly.num_groups", "num_groups"),
                     ("dragonfly.intergroup_per_router", "intergroup_per_router"), ("dragonfly.algorithm", "algorithm"), ("dragonfly.global_route_mode", "global_route_mode"), ("dragonfly.adaptive_threshold", "adaptive_threshold")]
        ...
        for g in range(_params["dragonfly.num_groups"]):
            ...
            for r in range(_params["dragonfly.routers_per_group"]):
                ...
                if router_num == 0:
                    ...
                    rtr.addParam("dragonfly.global_link_map",
                ...
                for p in range(_params["dragonfly.routers_per_group"]):
                    if p != r:
                        src = min(p, r)
                        dst = max(p, r)
                        rtr.addLink(getLink("link_g%dr%dr%d" % (
                            g, src, dst)), "port%d" % port, _params["link_lat"])
                        port = port + 1
...
class topoGraph(Topo):
    def __init__(self):
        Topo.__init__(self)
        self.topoKeys = ["topology", "debug", "num_ports", "flit_size", "link_bw", "xbar_bw",
                         "graph_file", "hosts_per_router", "total_routers", "num_vcs",
        ...
        self.topoOptKeys = ["xbar_arb", "num_vns", "vn_remap", "vn_remap_shm", "portcontrol.output_arb",
                            ...
                            "portcontrol.arbitration.arb_vcs", "link_bw:host", "link_bw:graph"]
        self.num_routers = 0
        self.edges = []
        self.adj = []
    ...
    def _load_graph(self, graph_file):
        ...
        if not records:
            sys.exit("graph topology: graph_file is empty")

        num_routers, declared_edges = records[0]
        if num_routers <= 0:
            sys.exit("graph topology: first line must be '<num_routers> <num_edges>'")

        edges = []
        for u, v in records[1:]:
            if u < 0 or v < 0 or u >= num_routers or v >= num_routers:
                sys.exit("graph topology: invalid edge (%d, %d) for %d routers" % (u, v, num_routers))
            if u == v:
                continue
            if u > v:
                u, v = v, u
        ...
        if declared_edges != len(edges):
            print("graph topology: declared %d edges, loaded %d unique non-self edges" %
        ...
        return num_routers, edges, adj

    def prepParams(self):
        graph_file = _params.get("graph_file", "")
        if not graph_file:
            sys.exit("graph topology requires graph_file")

        _params["graph_file"] = graph_file
        _params["hosts_per_router"] = int(_params.get("hosts_per_router", 1))
        _params["num_vcs"] = int(_params.get("num_vcs", 1))
        _params["debug"] = debug
        _params["topology"] = "merlin.graph"

        self.num_routers, self.edges, self.adj = self._load_graph(graph_file)

        max_radix = _params["hosts_per_router"]
        for rtr_id in range(self.num_routers):
            max_radix = max(max_radix, _params["hosts_per_router"] + len(self.adj[rtr_id]))

        _params["total_routers"] = self.num_routers
        _params["num_peers"] = self.num_routers * _params["hosts_per_router"]
        _params["router_radix"] = max_radix
        _params["num_ports"] = max_radix

    def build(self):
        links = dict()
        hosts_per_router = int(_params["hosts_per_router"])
        topo_params = _params.subset(["graph_file", "hosts_per_router", "total_routers", "num_vcs"])
        topo_params.update(_params.subsetWithRename([("graph.route_table_file", "route_table_file")]))
        
        for rtr_id in range(self.num_routers):
            radix = hosts_per_router + len(self.adj[rtr_id])
            rtr = self._instanceRouter(rtr_id, "merlin.hr_router")
            rtr.addParams(_params.subset(self.topoKeys, self.topoOptKeys))
            rtr.addParam("id", rtr_id)
            rtr.addParam("num_ports", radix)

            topology = rtr.setSubComponent("topology", "merlin.graph")
            topology.addParams(topo_params)

            port = 0
            for local_id in range(hosts_per_router):
                nodeID = (rtr_id * hosts_per_router) + local_id
                ep = self._getEndPoint(nodeID).build(nodeID, {})
                if ep:
                    nicLink = sst.Link("nic_%d_%d" % (rtr_id, local_id))
                    if self.bundleEndpoints:
                        nicLink.setNoCut()
                    nicLink.connect(ep, (rtr, "port%d" % port, _params["link_lat"]))
                port = port + 1

        neighbor_index = [{} for _ in range(self.num_routers)]
        for rtr_id in range(self.num_routers):
            for idx, neighbor in enumerate(self.adj[rtr_id]):
                neighbor_index[rtr_id][neighbor] = idx

        for u, v in self.edges:
            link_name = "link_%d_%d" % (u, v)
            if link_name not in links:
                links[link_name] = sst.Link(link_name)

            port_u = hosts_per_router + neighbor_index[u][v]
            port_v = hosts_per_router + neighbor_index[v][u]
            rtr_u = sst.findComponentByName(self.getRouterNameForId(u))
            rtr_v = sst.findComponentByName(self.getRouterNameForId(v))
            links[link_name].connect((rtr_u, "port%d" % port_u, _params["link_lat"]),
                                   (rtr_v, "port%d" % port_v, _params["link_lat"]))
...
class Pt2ptEndPoint(EndPoint):
    def __init__(self):
        ...
        self.epOptKeys.extend(
            ["linkcontrol", "stream_delays", "report_interval"])
...
class OfferedLoadEndPoint(EndPoint):
    def __init__(self):
        ...
        # self.enableAllStats = False;
        # self.statInterval = "0"
        self.epKeys.extend(["offered_load", "num_peers",
                           "link_bw", "message_size", "buffer_size", "pattern"])
...
class ShiftEndPoint(EndPoint):
    def __init__(self):
        ...
        self.epOptKeys.extend(
            ["checkerboard", "packets_to_send", "packet_size"])
...
class TrafficGenEndPoint(EndPoint):
    def __init__(self):
        ...
        self.epKeys.extend(["topology", "num_peers", "link_bw", "packets_to_send", "packet_size",
                           "message_rate", "PacketDest.pattern", "PacketDest.RangeMin", "PacketDest.RangeMax"])
    ...
    def prepParams(self):
        ...
        if _params["PacketDest.pattern"] == "NearestNeighbor":
            ...
            if not "PacketDest.NearestNeighbor.Size" in _params:
                _params["PacketDest.NearestNeighbor.Size"] = "%s %s %s" % (
...
if __name__ == "__main__":
    topos = dict([(1, topoTorus()), (2, topoFatTree()),
                 (3, topoDragonFly()), (4, topoSimple())])
    endpoints = dict(
