...
public:
  ...
  // RecDoubCollectiveEngine engine. Used as building blocks to implement
  // multidimensional collectives.
  class RecDoubCollectiveEngine {

  public:
    RecDoubCollectiveEngine(EmberRecDoubCollGenerator &gen, CollType coll_type,
  ...
  private:
    ...
    void computeReducescatterBlocksBitmap(int sender, int step,
                                          uint8_t *blocks_bitmap);
    void computeAllgatherBlocksBitmap(int sender, int step,
                                      uint8_t *blocks_bitmap);
    ...
    void processReceivedData(std::queue<EmberEvent *> &evQ,
                             CollType collective);
  ...
  // Abstract class that defines a generic recursive doubling collective
  // implementation running multiple collectives concurrently
  class RecDoubCollectiveRunner {
  public:
    RecDoubCollectiveRunner(EmberRecDoubCollGenerator &gen, int num_allreduces,
...
public:
  ...
  public:
    RecDoubCollective(EmberRecDoubCollGenerator &gen, uint dimensions_num,
                      uint ports, uint32_t count, uint32_t rank,
                      uint32_t comm_size, Communicator comm,
                      double aggregation_cost_ns = 0, bool nb = false,
                      bool sync = true, CollType coll_type = RECDOUB_ALLREDUCE,
...
public:
  SST_ELI_REGISTER_SUBCOMPONENT(
      EmberRecDoubCollGenerator, "ember", "EmberRecDoubMotif",
      SST_ELI_ELEMENT_VERSION(1, 0, 0),
      "EmberRecDoubMotif parent motif -- this is meant to be a superclass of "
      "EmberRecDoubMotif motifs",
