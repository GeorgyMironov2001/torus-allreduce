...
EmberRingAllreduceRevGenerator::EmberRingAllreduceRevGenerator(
  ...
  if (m_validate) {
    ...
    m_data_validation_send =
        (float *)memAlloc(sizeofDataType(FLOAT) * m_recvcount);
  ...
  for (int i = 0; i < concurrent; i++) {
    m_allreduce.push_back(new EmberRingAllreduceRev(
        *this, count, rank(), size(), GroupWorld, aggregation_cost_ns,
        !blocking, dimensions, dimensions_sizes, RING_ALLREDUCE, m_data));
  }
}
