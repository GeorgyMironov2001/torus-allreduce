...
private:
public:
  enum RingType { RING_ALLREDUCE = 0, RING_REDUCE_SCATTER, RING_ALLGATHER };

  // RingAllreduce engine. That's for 1D uni-directional. Used as building
  // blocks to implement 1D and 2D (bi-directional) allreduces.
  class RingAllreduce {

  public:
    RingAllreduce(EmberHxMeshGenerator &gen, RingType ring_type, float *dst,
                  uint32_t count, uint32_t vrank, uint32_t numproc,
                  uint32_t recvfrom, uint32_t sendto,
  ...
  private:
    ...
    void aggregate(std::queue<EmberEvent *> &evQ, float *a, float *b,
                   uint32_t num);
  ...
  public:
    EmberRingAllreduce(EmberHxMeshGenerator &gen, int num_allreduces,
...
public:
  ...
  public:
    EmberRandAlltoall(EmberHxMeshGenerator &gen, uint32_t sendcount,
                      uint32_t numblocks, uint32_t rank, uint32_t comm_size,
                      Communicator comm, bool nb, bool det);
    ~EmberRandAlltoall();
    bool progress(std::queue<EmberEvent *> &evQ);
    void reset();
  ...
  public:
    EmberImprovedAlltoall(EmberHxMeshGenerator &gen, uint32_t sendcount,
  ...
  public:
    EmberRingAllreduce05D(EmberHxMeshGenerator &gen, uint32_t count,
                          uint32_t rank, uint32_t comm_size, Communicator comm,
                          double aggregation_cost_ns = 0, bool nb = false,
                          RingType ring_type = RING_ALLREDUCE);
  ...
  public:
    EmberRingAllreduce1D(EmberHxMeshGenerator &gen, uint32_t count,
                         uint32_t rank, uint32_t comm_size, Communicator comm,
                         double aggregation_cost_ns = 0, bool nb = false,
                         RingType ring_type = RING_ALLREDUCE);
  ...
  public:
    EmberRingAllreduce2D(EmberHxMeshGenerator &gen, uint32_t count,
                         uint32_t rank, uint32_t comm_size, Communicator comm,
                         double aggregation_cost_ns = 0, bool nb = false,
                         uint32_t px = 1, RingType ring_type = RING_ALLREDUCE);
  ...
  private:
    enum allreduce_state_t { INIT, PHASE_1, PHASE_2, FINI };
  ...
  public:
    EmberRingAllreduce25D(EmberHxMeshGenerator &gen, uint32_t count,
                          uint32_t rank, uint32_t comm_size, Communicator comm,
                          double aggregation_cost_ns = 0, bool nb = false,
                          uint32_t px = 1, RingType ring_type = RING_ALLREDUCE);
  ...
  private:
    void validate();
    std::pair<uint, uint>
    getRowColFromRank(uint rank); // Assumes ranks are allocated row by row
    uint getRankFromRowCol(std::pair<uint, uint> rowcol);
    // Functions from the paper "Edge-disjoint Hamiltonian cycles in
  ...
  public:
    EmberRingAllreduceRev(EmberHxMeshGenerator &gen, uint32_t count,
                          uint32_t rank, uint32_t comm_size, Communicator comm,
                          double aggregation_cost_ns = 0, bool nb = false,
                          uint dimensions = 2, uint *dimensions_sizes = NULL,
                          RingType ring_type = RING_ALLREDUCE,
                          float *data = NULL);
...
public:
  SST_ELI_REGISTER_SUBCOMPONENT(EmberHxMeshGenerator, "ember",
                                        "HxMeshMotif",
                                        SST_ELI_ELEMENT_VERSION(1, 0, 0),
                                        "HxMesh parent motif -- this is meant "
                                        "to be a superclass of HxMesh motifs",
