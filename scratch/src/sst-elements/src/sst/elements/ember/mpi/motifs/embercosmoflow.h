...
public:
  SST_ELI_REGISTER_SUBCOMPONENT(EmberCosmoFlowGenerator, "ember",
                                "CosmoFlowMotif",
                                SST_ELI_ELEMENT_VERSION(1, 0, 0),
                                "Runs CosmoFlow", SST::Ember::EmberGenerator)

  SST_ELI_DOCUMENT_PARAMS({"arg.aggregation_cost_ns", "Cost to sum two floats",
                           "0.01"},
                          {"arg.all_reduce_type",
                           "All Reduce Type (05D | 2D | 25D)", "05D"}, )
...
private:
  ...
  EmberRingAllreduce *makeAllGather(uint32_t count, uint32_t rank,
                                    uint32_t comm_size, Communicator comm);
  EmberRingAllreduce *makeReduceScatter(uint32_t count, uint32_t rank,
                                        uint32_t comm_size, Communicator comm);
  EmberRingAllreduce *makeIAllreduce(uint32_t count, uint32_t rank,
                                     uint32_t comm_size, Communicator comm);
  void temp_fix_all_reduce_size();

private:
  enum cosmoflow_state_t { SETUP = 0, ITERATE };

  enum iteration_state_t { IT_FORWARD = 0, IT_BACKWARD };

  enum fwd_state_t { FWD_INIT = 0, FWD_CONV, FWD_CONV_WAIT, FWD_ALLGATHER };
...
private:
  int m_conv_fwd_halo_sizes[NUM_Conv_L - 1] = {2097152, 1048576, 524288,
                                               262144};
  ...
  int m_allreduce_sizes[NUM_L - 2] = {1050737, 3539456, 884992,
                                      221312,  55360,   3488};
  ...
  // in ns
  int m_fwd_rt_per_layer[NUM_L] = {6567 * 1000, 13135 * 1000, 6567 * 1000,
                                   3283 * 1000, 1641 * 1000,  5 * 1000,
                                   3 * 1000,    1 * 1000};
  int m_bwd_rt_per_layer[NUM_L] = {2 * 1000,     6 * 1000,    10 * 1000,
                                   3283 * 1000,  6567 * 1000, 13135 * 1000,
                                   26270 * 1000, 13135 * 1000};
