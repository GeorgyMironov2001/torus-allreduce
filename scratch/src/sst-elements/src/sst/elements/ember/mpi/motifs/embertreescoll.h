...
struct TreeSpec {
  ...
  explicit TreeSpec(int center_, int dim_size_,
                    const std::vector<TreeEdge> &edges_)
      : center(center_), dim_size(dim_size_), edges(edges_) {
    inside_peers.assign(dim_size * dim_size, std::vector<TreeEdge>());
    outside_peers.assign(dim_size * dim_size, std::vector<TreeEdge>());
...
public:
  ...
  public:
    TreesCollective(EmberTreesCollGenerator &gen, uint dimensions_num,
                    ...
                    uint32_t comm_size, Communicator comm, std::vector<TreeSpec> tree_specs,
