...
public:
  SST_ELI_DOCUMENT_PARAMS({"rankmapper", "Sets the rank mapper module", ""}, )
  /* PARAMS
          rankmap.*
  */

  EmberMessagePassingGenerator(ComponentId_t id, Params &params,
                               std::string name = "");
...
protected:
  ...
  void getPosition(int32_t rank, int32_t px, int32_t py, int32_t *myX,
                   int32_t *myY) {
  ...
  int32_t convertPositionToRank(int32_t px, int32_t py, int32_t myX,
                                int32_t myY) {
  ...
  void op_free(ReductionOperation op) { Hermes::MP::Op_free(op); }
