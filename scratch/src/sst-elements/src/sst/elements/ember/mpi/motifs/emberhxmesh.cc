...
EmberHxMeshGenerator::RingAllreduce::RingAllreduce(
    EmberHxMeshGenerator &gen, RingType ring_type, float *dst, uint32_t count,
    uint32_t vrank, uint32_t numproc, uint32_t recvfrom, uint32_t sendto,
    double aggregation_cost_ns, Communicator comm, bool validate)
    : m_gen(gen), m_count(count), m_dst(dst), m_r(vrank), m_p(numproc),
      m_recvfrom(recvfrom), m_sendto(sendto),
      m_aggregation_cost_ns(aggregation_cost_ns), m_data_sent(0), m_comm(comm),
      m_enabled(true), m_validate(validate) {
...
bool EmberHxMeshGenerator::RingAllreduce::isEnabled() { return m_enabled; }

bool EmberHxMeshGenerator::RingAllreduce::progress(
...
float *EmberHxMeshGenerator::RingAllreduce::getBuff() { return m_dst; }
...
void EmberHxMeshGenerator::RingAllreduce::aggregate(
    std::queue<EmberEvent *> &evQ, float *a, float *b, uint32_t num) {
...
uint32_t
EmberHxMeshGenerator::RingAllreduce::getBlockOffset(uint32_t block_idx) {
  if (m_count < m_p)
    return 0;
...
bool EmberHxMeshGenerator::RingAllreduce::reduceScatter(
    std::queue<EmberEvent *> &evQ) {
  ...
  if (m_i == 0) {
    if (m_p > 1) {
      ...
      m_gen.enQ_irecv(evQ, &m_tmp[0], recv_block_size, FLOAT, m_recvfrom,
                      m_tag1, m_comm, &m_req_recv);
      DPRINTF("[%d] RS init: recv from %d; recv_block_idx: %d; "
              "recv_block_size: %d; tag: %d;\n",
              m_gen.rank(), m_recvfrom, recv_block_idx, recv_block_size,
              m_tag1);
      m_waiting_recv = true;
      ...
      DPRINTF("[%d] RS init: send to %d; send_block_idx: %d; buff_offset: %d; "
              "send_buff_size: %d; tag: %d;\n",
              m_gen.rank(), m_sendto, send_block_idx, send_buff_offset,
              send_buff_size, m_tag1);
      m_gen.enQ_isend(evQ, &m_dst[send_buff_offset], send_buff_size, FLOAT,
                      m_sendto, m_tag1, m_comm, &m_req_send);
      m_data_sent += send_buff_size * m_gen.sizeofDataType(FLOAT);
    }
  } else {
    // if we are here, it means that the prev recv has been matched.
    // now we aggregate & forward the recvd block + post the recv for the next
    // one
    m_waiting_recv = false;
    ...
    // printf("[%d] RS agg: m_i: %d; m_p: %d; recv from %d; recvoffset: %d (%p);
    // m_blocksz: %d\n", m_gen.rank(), m_i, m_p, m_recvfrom, recvoffset,
    // &(m_dst[recvoffset]), m_blocksz);
    ...
    if (m_i < m_p - 1) {
      DPRINTF("[%d] RS: send to %d; recv_block_offset: %d; recv_block_offset: "
              "%d; count: %d; tag: %d\n",
              m_gen.rank(), m_sendto, recv_block_idx, recv_block_offset,
              recv_block_size, m_tag1);
      m_gen.enQ_isend(evQ, &m_dst[recv_block_offset], recv_block_size, FLOAT,
                      m_sendto, m_tag1, m_comm, &m_req_send);
...
bool EmberHxMeshGenerator::RingAllreduce::allGather(
    std::queue<EmberEvent *> &evQ) {
  ...
  if (m_i == 0) {
    if (m_p > 1) {
      ...
      uint32_t recv_block_size = getBlockSize(recv_block_idx);
      uint32_t recv_block_offset = getBlockOffset(recv_block_idx);
      DPRINTF("[%d] AG init: recv from %d; recv_block_idx: %d; "
              "recv_block_offset: %d; recv_block_size: %d; tag: %d;\n",
              m_gen.rank(), m_recvfrom, recv_block_idx, recv_block_offset,
              recv_block_size, m_tag2);
      m_gen.enQ_irecv(evQ, &m_dst[recv_block_offset], recv_block_size, FLOAT,
                      m_recvfrom, m_tag2, m_comm, &m_req_recv);
      m_waiting_recv = true;
      ...
      DPRINTF("[%d] AG init: send to %d; send_block_idx: %d; "
              "send_block_offset: %d; send_block_size: %d; tag: %d;\n",
              m_gen.rank(), m_sendto, send_block_idx, send_block_offset,
              send_block_size, m_tag2);
      m_gen.enQ_isend(evQ, &m_dst[send_block_offset], send_block_size, FLOAT,
                      m_sendto, m_tag2, m_comm, &m_req_send);
    ...
    if (m_i < m_p - 1) {
      ...
      uint32_t recv_block_size = getBlockSize(recv_block_idx);
      uint32_t recv_block_offset = getBlockOffset(recv_block_idx);
      DPRINTF("[%d] AG: recv from %d; recv_block_idx: %d; recv_block_offset: "
              "%d; recv_block_size: %d; tag: %d;\n",
              m_gen.rank(), m_recvfrom, recv_block_idx, recv_block_offset,
              recv_block_size, m_tag2);
      m_gen.enQ_irecv(evQ, &m_dst[recv_block_offset], recv_block_size, FLOAT,
                      m_recvfrom, m_tag2, m_comm, &m_req_recv);
      ...
      uint32_t send_block_size = getBlockSize(send_block_idx);
      uint32_t send_block_offset = getBlockOffset(send_block_idx);
      DPRINTF("[%d] AG: send to %d; send_block_idx: %d; send_block_offset: %d; "
              "send_block_size: %d; tag: %d;\n",
              m_gen.rank(), m_sendto, send_block_idx, send_block_offset,
              send_block_size, m_tag2);
      m_gen.enQ_isend(evQ, &m_dst[send_block_offset], send_block_size, FLOAT,
                      m_sendto, m_tag2, m_comm, &m_req_send);
...
/******* RingAllreduceBase *******/
EmberHxMeshGenerator::EmberRingAllreduce::EmberRingAllreduce(
    EmberHxMeshGenerator &gen, int num_allreduces, uint32_t count,
    uint32_t rank, uint32_t comm_size, bool nonblocking)
    : m_gen(gen), m_nb(nonblocking), m_has_new_recv(false), m_count(count),
      m_r(rank), m_p(comm_size) {
...
bool EmberHxMeshGenerator::EmberRingAllreduce::progress_phase(
    std::queue<EmberEvent *> &evQ) {
  ...
  for (auto &allreduce : m_allreduces) {
    ...
    if (allreduce.hasPendingRecv()) {
      ...
      // we need to do this because SST doesn't like invalid recv handles in the
      // waitany :(
      // printf("[%d] allreduce %p waiting for %p\n", m_gen.rank(), &allreduce,
...
EmberHxMeshGenerator::EmberRingAllreduce05D::EmberRingAllreduce05D(
    EmberHxMeshGenerator &gen, uint32_t count, uint32_t rank,
    uint32_t comm_size, Communicator comm, double aggregation_cost_ns, bool nb,
    RingType ring_type)
  ...
  // Allreduces
  m_allreduces.push_back(RingAllreduce(m_gen, ring_type, m_dstx_1, m_count, m_r,
                                       m_p, recvfrom, sendto,
                                       aggregation_cost_ns, comm));

  printf("Rank: %d; Grid: %dx1; m_count: %d; sendto: %d; recvfrom: %d; "
         "Blocking: %d\n",
         m_r, m_p, m_count, sendto, recvfrom, (int)m_nb);
}

bool EmberHxMeshGenerator::EmberRingAllreduce05D::progress(
    std::queue<EmberEvent *> &evQ) {
  switch (m_state) {
  case INIT:
...
EmberHxMeshGenerator::EmberRingAllreduce1D::EmberRingAllreduce1D(
  ...
  // Allreduces
  m_allreduces.push_back(RingAllreduce(m_gen, ring_type, m_dstx_1, m_count / 2,
                                       m_r, m_p, recvfrom, sendto,
                                       aggregation_cost_ns, comm));
  m_allreduces.push_back(RingAllreduce(m_gen, ring_type, m_dstx_2, m_count / 2,
                                       m_p - m_r, m_p, sendto, recvfrom,
                                       aggregation_cost_ns, comm));
...
bool EmberHxMeshGenerator::EmberRingAllreduce1D::progress(
    std::queue<EmberEvent *> &evQ) {
  switch (m_state) {
  case INIT:
    m_gen.enQ_getTime(evQ, &m_start_time);
...
EmberHxMeshGenerator::EmberRingAllreduce2D::EmberRingAllreduce2D(
    EmberHxMeshGenerator &gen, uint32_t count, uint32_t rank,
    uint32_t comm_size, Communicator comm, double aggregation_cost_ns, bool nb,
    uint32_t px, RingType ring_type)
  ...
  // Allreduces
  m_allreduces.push_back(RingAllreduce(m_gen, ring_type, m_dstx_1, m_count / 4,
                                       ry, m_px, recvfrom_x, sendto_x,
                                       aggregation_cost_ns, comm));
  m_allreduces.push_back(RingAllreduce(m_gen, ring_type, m_dstx_2, m_count / 4,
                                       m_px - ry, m_px, sendto_x, recvfrom_x,
                                       aggregation_cost_ns, comm));
  m_allreduces.push_back(RingAllreduce(m_gen, ring_type, m_dsty_1, m_count / 4,
                                       rx, m_py, recvfrom_y, sendto_y,
                                       aggregation_cost_ns, comm));
  m_allreduces.push_back(RingAllreduce(m_gen, ring_type, m_dsty_2, m_count / 4,
                                       m_py - rx, m_py, sendto_y, recvfrom_y,
                                       aggregation_cost_ns, comm));
  ...
  printf("Rank: %d; Grid: %dx%d; m_count: %d; sendto_x: %d; recvfrom_x: %d; "
         "sendto_y: %d; recvfrom_y: %d; Blocking: %d\n",
         m_r, m_py, m_px, m_count, sendto_x, recvfrom_x, sendto_y, recvfrom_y,
         (int)!m_nb);
}

bool EmberHxMeshGenerator::EmberRingAllreduce2D::progress(
    std::queue<EmberEvent *> &evQ) {
  switch (m_state) {
  case INIT:
    m_gen.enQ_getTime(evQ, &m_start_time);
...
EmberHxMeshGenerator::EmberRingAllreduce25D::EmberRingAllreduce25D(
  ...
  printf("K0(px) is %d - K1 is %d - Size %d - Job ID %d", k0, k1, m_gen.size(),
         m_gen.getJobId());
  fflush(stdout);
  ...
  assert(m_dst_1 != NULL && m_dst_2 != NULL && m_dst_3 != NULL &&
         m_dst_4 != NULL);
  for (int i = 0; i < m_count / 4; i++) {
    m_dst_1[i] = i;
    m_dst_2[i] = 2 * i;
  ...
  // Allreduces
  m_allreduces.push_back(RingAllreduce(m_gen, ring_type, m_dst_1, m_count / 4,
                                       m_r, m_p, neigh0a, neigh0b,
                                       aggregation_cost_ns, comm));
  m_allreduces.push_back(RingAllreduce(m_gen, ring_type, m_dst_2, m_count / 4,
                                       m_p - m_r, m_p, neigh0b, neigh0a,
                                       aggregation_cost_ns, comm));
  m_allreduces.push_back(RingAllreduce(m_gen, ring_type, m_dst_3, m_count / 4,
                                       m_r, m_p, neigh1a, neigh1b,
                                       aggregation_cost_ns, comm));
  m_allreduces.push_back(RingAllreduce(m_gen, ring_type, m_dst_4, m_count / 4,
                                       m_p - m_r, m_p, neigh1b, neigh1a,
                                       aggregation_cost_ns, comm));
  ...
  printf("Rank: %d; Grid: %dx%d; m_count: %d; neigh0a: %d; neigh0b: %d; "
         "neigh1a: %d; neigh1b: %d; Blocking: %d\n",
         m_r, k0, k1, m_count, neigh0a, neigh0b, neigh1a, neigh1b, (int)m_nb);
}

bool EmberHxMeshGenerator::EmberRingAllreduce25D::progress(
    std::queue<EmberEvent *> &evQ) {
  switch (m_state) {
  case INIT:
    m_gen.enQ_getTime(evQ, &m_start_time);
...
std::pair<uint, uint>
EmberHxMeshGenerator::EmberRingAllreduce25D::getRowColFromRank(uint rank) {
  return std::pair<uint, uint>(rank / k0, rank % k0);
}

uint EmberHxMeshGenerator::EmberRingAllreduce25D::getRankFromRowCol(
    std::pair<uint, uint> rowcol) {
  return rowcol.first * k0 + rowcol.second;
}

std::pair<uint, uint>
EmberHxMeshGenerator::EmberRingAllreduce25D::getx1x0(uint X) {
  // return std::pair<uint, uint>(std::floor(X / k1), X % k0);
  return std::pair<uint, uint>(X / k0, X % k0);
}

std::pair<uint, uint>
EmberHxMeshGenerator::EmberRingAllreduce25D::getf0(uint X) {
  auto x1x0 = getx1x0(X);
  uint x1 = x1x0.first;
  uint x0 = x1x0.second;
...
std::pair<uint, uint>
EmberHxMeshGenerator::EmberRingAllreduce25D::getf1(uint X) {
  auto x1x0 = getx1x0(X);
  uint x1 = x1x0.first;
  uint x0 = x1x0.second;
...
std::pair<uint, uint>
EmberHxMeshGenerator::EmberRingAllreduce25D::getNeighRing0(uint rank) {
  ...
  Xpost = (Xrank + 1) % (k0 * k1);
  return std::pair<uint, uint>(getRankFromRowCol(getf0(Xpre)),
                               getRankFromRowCol(getf0(Xpost)));
}

std::pair<uint, uint>
EmberHxMeshGenerator::EmberRingAllreduce25D::getNeighRing1(uint rank) {
  auto rowcol = getRowColFromRank(rank);
  uint row = rowcol.first;
  uint col = rowcol.second;
  ...
  Xpost = (Xrank + 1) % (k0 * k1);
  return std::pair<uint, uint>(getRankFromRowCol(getf1(Xpre)),
                               getRankFromRowCol(getf1(Xpost)));
}
...
static void getCoordFromId(int id, uint *dimensions, uint dimensions_num,
                           int *coord, int m_p) {
  int nnodes = m_p;
...
EmberHxMeshGenerator::EmberRingAllreduceRev::EmberRingAllreduceRev(
    EmberHxMeshGenerator &gen, uint32_t count, uint32_t rank,
    uint32_t comm_size, Communicator comm, double aggregation_cost_ns, bool nb,
    uint dimensions, uint *dimensions_sizes, RingType ring_type, float *data)
    : EmberRingAllreduce(gen, 2 * dimensions * 2 * dimensions, count, rank,
                         comm_size, nb),
      m_state(0), m_dimensions(dimensions),
      m_dimensions_sizes(dimensions_sizes), m_validate(data != NULL),
      m_data(data) {
  ...
  for (int s = 0; s < m_dimensions; s++) {
    for (int i = 0; i < m_dimensions; i++) {
      ...
      size_t offset_r =
          (2 * i + 1) * (m_count / (2 * m_dimensions)) + offsets[i];
      assert(offset_l + data_size[i] <=
             (2 * i + 1) * (m_count / (2 * m_dimensions)));
      ...
      */
      ...
      reducescatters.push_back(RingAllreduce(
          m_gen, RING_REDUCE_SCATTER, data + offset_l, data_size[i],
          relcoord[current_d], m_dimensions_sizes[current_d],
          recvfrom[current_d], sendto[current_d], aggregation_cost_ns, comm,
          m_validate));
      if (m_validate) {
        reducescatters.push_back(RingAllreduce(
            m_gen, RING_REDUCE_SCATTER, data + offset_r, data_size[i],
            relcoord[current_d], m_dimensions_sizes[current_d],
            recvfrom[current_d], sendto[current_d], aggregation_cost_ns, comm,
            m_validate));
      } else {
        reducescatters.push_back(RingAllreduce(
            m_gen, RING_REDUCE_SCATTER, data + offset_r, data_size[i],
            m_dimensions_sizes[current_d] - relcoord[current_d] - 1,
            m_dimensions_sizes[current_d], sendto[current_d],
            recvfrom[current_d], aggregation_cost_ns, comm, m_validate));
      }

      allgathers.push_back(
          RingAllreduce(m_gen, RING_ALLGATHER, data + offset_l, data_size[i],
                        relcoord[current_d], m_dimensions_sizes[current_d],
                        recvfrom[current_d], sendto[current_d],
                        aggregation_cost_ns, comm, m_validate));
      if (m_validate) {
        allgathers.push_back(
            RingAllreduce(m_gen, RING_ALLGATHER, data + offset_r, data_size[i],
                          relcoord[current_d], m_dimensions_sizes[current_d],
                          recvfrom[current_d], sendto[current_d],
                          aggregation_cost_ns, comm, m_validate));
      } else {
        allgathers.push_back(RingAllreduce(
            m_gen, RING_ALLGATHER, data + offset_r, data_size[i],
            m_dimensions_sizes[current_d] - relcoord[current_d] - 1,
            m_dimensions_sizes[current_d], sendto[current_d],
            recvfrom[current_d], aggregation_cost_ns, comm, m_validate));
      }
...
bool EmberHxMeshGenerator::EmberRingAllreduceRev::progress(
    std::queue<EmberEvent *> &evQ) {
  while (true) {
...
void EmberHxMeshGenerator::EmberRingAllreduceRev::validate() { assert(0); }
...
EmberHxMeshGenerator::EmberRandAlltoall::EmberRandAlltoall(
    EmberHxMeshGenerator &gen, uint32_t sendcount, uint32_t numblocks,
    uint32_t rank, uint32_t comm_size, Communicator comm, bool nb, bool det)
    : m_gen(gen), m_r(rank), m_p(comm_size), m_sendcount(sendcount),
      m_comm(comm), m_numblocks(numblocks), m_tot_blocks(0), m_tag(0),
      m_send_block_idx(NULL), m_it(0) {
  ...
  m_req_recv =
      (MessageRequest *)malloc(sizeof(MessageRequest) * blocks_to_exchange);
  m_req_send =
      (MessageRequest *)malloc(sizeof(MessageRequest) * blocks_to_exchange);
}
...
bool EmberHxMeshGenerator::EmberRandAlltoall::progress(
    std::queue<EmberEvent *> &evQ) {
  int blocks_to_exchange = (m_p - 1) * m_numblocks;
  ...
  if (m_it == 0) {
    ...
    for (int i = 1; i <= blocks_to_exchange; i++) {
      ...
      m_gen.enQ_irecv(evQ, NULL, block_size, FLOAT, recvfrom, m_tag, m_comm,
                      &m_req_recv[recvs]);
      recvs++;
    }
    int sends = 0;
    for (int i = 0; i < m_tot_blocks; i++) {
      ...
      if (sendto != m_r) {
        ...
        m_gen.enQ_isend(evQ, NULL, block_size, FLOAT, sendto, m_tag, m_comm,
                        &m_req_send[sends]);
        sends++;
      }
    }
...
EmberHxMeshGenerator::EmberImprovedAlltoall::EmberImprovedAlltoall(
    EmberHxMeshGenerator &gen, uint32_t sendcount, uint32_t rank,
...
bool EmberHxMeshGenerator::EmberImprovedAlltoall::progress(
    std::queue<EmberEvent *> &evQ) {
...
EmberHxMeshGenerator::EmberHxMeshGenerator(SST::ComponentId_t id,
                                           Params &params)
