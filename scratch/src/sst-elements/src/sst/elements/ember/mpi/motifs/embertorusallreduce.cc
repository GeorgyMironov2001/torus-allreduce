#include "embertorusallreduce.h"
#include <sst_config.h>

using namespace SST::Ember;

EmberTorusAllreduceGenerator::EmberTorusAllreduceGenerator(
    SST::ComponentId_t id, Params &params)
    : EmberTorusCollGenerator(id, params, "TorusAllreduce") {
  ...
  m_allreduce = new TorusCollective(*this, dimensions, ports, recvcount, rank(),
                                    size(), GroupWorld, aggregation_cost_ns,
                                    !blocking, sync, TORUS_ALLREDUCE, m_data,
...
bool EmberTorusAllreduceGenerator::generate(std::queue<EmberEvent *> &evQ) {
