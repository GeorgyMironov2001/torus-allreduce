...
public:
  SST_ELI_REGISTER_SUBCOMPONENT(EmberRingReduceScatter1DGenerator, "ember",
                                "RingReduceScatter1DMotif",
                                SST_ELI_ELEMENT_VERSION(1, 0, 0),
                                "Performs a RingReduceScatter1D operation with "
                                "type set to FLOAT and operation SUM",
                                SST::Ember::EmberGenerator)

  SST_ELI_DOCUMENT_PARAMS(
      {"arg.aggregation_cost_ns", "Cost to sum two floats", "0.01"},
      {"arg.recvcount",
       "Sets the block size (buffers is n*recvcount, n=num processes)", "1"},
