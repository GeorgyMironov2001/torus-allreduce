...
public:
  EmberIRecvEvent(MP::Interface &api, Output *output,
                  EmberEventTimeStatistic *stat, const Hermes::MemAddr &payload,
                  uint32_t count, PayloadDataType dtype, RankID dest,
                  uint32_t tag, Communicator group, MessageRequest *req)
      : EmberMPIEvent(api, output, stat), m_payload(payload), m_count(count),
        m_dtype(dtype), m_dest(dest), m_tag(tag), m_group(group), m_req(req) {}
  ...
  void issue(uint64_t time, FOO *functor) {
    ...
    m_api.irecv(m_payload, m_count, m_dtype, m_dest, m_tag, m_group, m_req,
                functor);
  }
