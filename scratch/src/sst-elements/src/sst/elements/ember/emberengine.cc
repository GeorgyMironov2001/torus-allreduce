...
EmberEngine::EmberEngine(SST::ComponentId_t id, SST::Params &params)
    : Component(id), currentMotif(0), m_motifDone(false),
  ...
  if ("" != motifLogFile) {
    m_motifLogger =
        loadComponentExtension<EmberMotifLog>(motifLogFile, m_jobId);
  } else {
  ...
  m_generator =
      initMotif(motifParams[0], m_apiMap, m_jobId, currentMotif, m_nodePerf);
  assert(m_generator);
...
EmberEngine::ApiMap EmberEngine::createApiMap(OS *os, SST::Component *owner,
                                              SST::Params params) {
  ApiMap tmp;
  ...
  while (1) {
    ...
    Hermes::Interface *api = loadAnonymousSubComponent<Interface>(
        moduleName, "", 0, ComponentInfo::SHARE_NONE, modParams);
...
EmberGenerator *EmberEngine::initMotif(SST::Params params, const ApiMap &apiMap,
  ...
  if (gentype.empty()) {
    output.fatal(CALL_INFO, -1,
                 "Error: You did not specify a generator"
                 "or Ember to use\n");
  } else {
    ...
    params.insert("_enginePtr",
                  std::to_string(reinterpret_cast<uint64_t>(this)), true);

    gen = loadAnonymousSubComponent<EmberGenerator>(
        gentype, "", 0, ComponentInfo::SHARE_NONE, params);
...
void EmberEngine::setup() {
  ...
  std::ostringstream prefix;
  prefix << "@t:" << m_jobId << ":" << m_os->getRank()
         << ":EmberEngine:@p:@l: ";
  // std::cout << "@t:" << m_jobId << ":" << m_os->getRank() <<
  // ":EmberEngine:@p:@l: " << std::endl; //NetworkSim
...
void EmberEngine::issueNextEvent(uint64_t nanoDelay) {
  ...
  output.debug(CALL_INFO, 8, ENGINE_MASK,
               "Engine issuing next event with delay %" PRIu64 "\n", nanoDelay);
  // printf("evQueue.size() = %ld, rank = %d\n", evQueue.size(),
  // m_os->getRank());
  while (evQueue.empty()) {
    ...
    if (evQueue.empty()) {
      ...
      // output.verbose(CALL_INFO, 1, MOTIF_START_STOP_MASK, "Motif finished:
      ...
      if (++currentMotif == motifParams.size()) {
        ...
        m_generator = initMotif(motifParams[currentMotif], m_apiMap, m_jobId,
                                currentMotif, m_nodePerf);
        assert(m_generator);
        if (NULL != m_motifLogger) {
          m_motifLogger->logMotifStart(currentMotif);
        }
        // output.verbose(CALL_INFO, 1, MOTIF_START_STOP_MASK, "Motif starting:
        // %s\n",m_generator->getMotifName().c_str());
  ...
  evQueue.pop();
  // printf("nextEv = %s, rank = %d, state = %s\n", nextEv->getName().c_str(),
  //        m_os->getRank(), nextEv->stateName(nextEv->state()).c_str());
  // fflush(stdout);
...
void EmberEngine::handleEvent(Event *ev) {
  ...
  case EmberEvent::IssueCallback:
    eEv->issue(getCurrentSimTimeNano(),
               std::bind(&EmberEngine::completeCallback, this, eEv,
                         std::placeholders::_1));
    break;

  case EmberEvent::IssueCallbackPtr: {
    Callback *callback = new Callback;
    *callback = std::bind(&EmberEngine::completeCallback, this, eEv,
                          std::placeholders::_1);
...
EmberEngine::EmberEngine() : Component(-1) {
