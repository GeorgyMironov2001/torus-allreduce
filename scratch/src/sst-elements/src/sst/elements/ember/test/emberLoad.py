...
if simConfig:
    try:
        ...
        sys.exit(
            'Failed: could not import sim configuration `{0}`'.format(simConfig))

    workFlow, numNodes, numCores, nicsPerNode = config.getWorkFlow(
        motifDefaults)
...
if detailedModelName:
    ...
    try:
        ...
        sys.exit('Failed: could not import detailed model `{0}`'.format(
            detailedModelName))

    try:
        modelParams = __import__(detailedModelParams, fromlist=[''])
    except:
        sys.exit('Failed: could not import detailed model params `{0}`'.format(
...
if rtrArb:
    print("EMBER: network: topology={0} shape={1} arbitration={2}".format(
...
if int(numNodes) > int(topoInfo.getNumNodes()):
    sys.exit("need more nodes want " + str(numNodes) +
             ", have " + str(topoInfo.getNumNodes()))

print("EMBER: numNodes={0} numNics={1}".format(
    numNodes, topoInfo.getNumNodes()))
...
if jobid > 0 and rndmPlacement:
    ...
    nidList = ""
    nids = random.sample(
        list(range(int(topoInfo.getNumNodes()))), int(numNodes))
    # nids.sort()
...
if rndmPlacement and bgPercentage > 0:
    ...
    while (count < bgMotifs):
        workFlow = []
        nidList = "-nidList=" + \
            str(emptyNids[count * 2]) + "," + str(emptyNids[count * 2 + 1])
        motif = dict.copy(motifDefaults)
        ...
        motif['cmd'] = nidList + " TrafficGen mean="+str(bgMean) + " stddev=" + \
            str(bgStddev) + " messageSize=" + \
            str(bgMsgSize) + " startDelay=" + str(y * 500)
        workFlow.append(motif)
...
for a in params['network']:
    ...
    if key in networkParams:
        print("override networkParams {0}={1} with {2}".format(
...
for a in params['nic']:
    key, value = a.split("=")
    if key in nicParams:
        print("override nicParams {0}={1} with {2}".format(
            key, nicParams[key], value))
    else:
...
for a in params['ember']:
    ...
    if key in emberParams:
        print("override emberParams {0}={1} with {2}".format(
            key, emberParams[key], value))
    else:
...
for a in params['hermes']:
    ...
    if key in hermesParams:
        print("override hermesParams {0}={1} with {2}".format(
...
baseNicParams = {
    "packetSize": networkParams['packetSize'],
    "link_bw": nicParams['link_bw'],
    "input_buf_size": networkParams['input_buf_size'],
    "output_buf_size": networkParams['output_buf_size'],
    "module": nicParams['module']
}
loadInfo = LoadInfo(topoInfo.getNumNodes(),
                    topoInfo.getNicsPerNode(), baseNicParams, epParams)
...
if statsModuleName:
    ...
    try:
        ...
        sys.exit('Failed: could not import statsistics module `{0}`'.format(
            statsModuleName))
