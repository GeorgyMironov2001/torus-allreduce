...
def main(args):
    ...
    nodes64 = "../../allreduce/output/torus_8x8/diff"
    nodes256 = "../../allreduce/output/torus_16x16/diff"
    nodes1024 = "../../allreduce/output/torus_32x32/diff"
    nodes4096 = "../../allreduce/output/torus_64x64/diff"
    nodes16384 = "../../allreduce/output/torus_128x128/diff"
    ...
    sns.lineplot(x = "X", y = "Y", data=data_plot64, label="64", marker='o', legend=False, color=palette[0], linewidth=2.5, markersize=9)

    performance_diff = []
    with open(nodes256) as f:
        performance_diff = f.read().splitlines()
    performance_diff = [int(float(x)) for x in performance_diff][0:len(allreduce_sizes_all)]
    data_plot256 = pd.DataFrame({"X":allreduce_sizes_all, "Y":performance_diff})
    sns.lineplot(x = "X", y = "Y", data=data_plot256, label="256", marker='P', legend=False, color=palette[1], linewidth=2.5, markersize=9)
    
    performance_diff = []
    with open(nodes1024) as f:
        performance_diff = f.read().splitlines()
    performance_diff = [int(float(x)) for x in performance_diff][0:len(allreduce_sizes_all)]
    data_plot1024 = pd.DataFrame({"X":allreduce_sizes_all, "Y":performance_diff})
    sns.lineplot(x = "X", y = "Y", data=data_plot1024, label="1024", marker='X', legend=False, color=palette[2], linewidth=2.5, markersize=9)

    with open(nodes4096) as f:
        performance_diff = f.read().splitlines()
    performance_diff = [int(float(x)) for x in performance_diff][0:len(allreduce_sizes_all)]
    data_plot4096 = pd.DataFrame({"X":allreduce_sizes_all, "Y":performance_diff})
    sns.lineplot(x = "X", y = "Y", data=data_plot4096, label="4096", marker='D', legend=False, color=palette[3], linewidth=2.5, markersize=9)

    with open(nodes16384) as f:
        performance_diff = f.read().splitlines()
    performance_diff = [int(float(x)) for x in performance_diff][0:len(allreduce_sizes_all)]
    data_plot4096 = pd.DataFrame({"X":allreduce_sizes_all, "Y":performance_diff})
    sns.lineplot(x = "X", y = "Y", data=data_plot4096, label="16384", marker='^', legend=False, color=palette[4], linewidth=2.5, markersize=9)
    ...
    if (ymin > ymax):
        ...
        ax.set_ylim([-30, ymax])
    ax.axhspan(ymin=-30  - 20, ymax=0, facecolor="red", alpha=0.15)
    ...
    #ax.set_title('Analyzing the scaling of Swing on Torus'.format(args.topo), fontdict={'fontsize': 17})
